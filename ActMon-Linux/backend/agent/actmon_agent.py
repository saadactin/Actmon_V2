...
def _dbq_open(dbtype, host, port, user, pw, dbname, service_name=None, sid=None):
    ...
    if "oracle" in dbtype:
        ...
        # Oracle identifies the target via EITHER a service_name (EZConnect "/service")
        # OR a SID (EZConnect ":SID") — these are not interchangeable syntax, and a
        # SID-only connection (common for EBS/legacy installs) given the service_name
        # slash form fails with ORA-12514 (listener doesn't know that "service"). Prefer
        # service_name, then SID, then fall back to the generic database name/"XE".
        if service_name:
            dsn = "%s:%s/%s" % (host, port or 1521, service_name)
        elif sid:
            dsn = "%s:%s:%s" % (host, port or 1521, sid)
        else:
            dsn = "%s:%s/%s" % (host, port or 1521, dbname or "XE")
...
def _ora_init_thick(oracledb):
    ...
    # LD_LIBRARY_PATH is how most manual Instant Client installs (esp. on Ubuntu/Debian,
    # which has no RPM-managed /usr/lib/oracle convention) tell a process where the
    # client libs live — honor it before falling back to guessed locations.
    for d in (os.environ.get("LD_LIBRARY_PATH") or "").split(os.pathsep):
        if d:
            cands.append(d)
    for pat in (
        # RHEL/Oracle Linux — full DB Home / Grid Home (rpm or manual install)
        "/u01/app/oracle/product/*/db*/lib", "/u01/app/*/grid/lib",
        "/opt/oracle/product/*/db*/lib",
        # Instant Client — manual zip install (Oracle Linux and Ubuntu alike)
        "/opt/oracle/instantclient*", "/usr/local/instantclient*", "/usr/local/lib/instantclient*",
        # Oracle Linux / RHEL — Instant Client RPM
        "/usr/lib/oracle/*/client*/lib", "/usr/lib/oracle/*/client64/lib",
        # Ubuntu/Debian — Instant Client unpacked under the multiarch lib dir, or apt/alien-converted rpm
        "/usr/lib/x86_64-linux-gnu/oracle/*/client*/lib", "/usr/lib/x86_64-linux-gnu/instantclient*",
    ):
...
def _dbq_get_conn(key, dbtype, host, port, user, pw, dbname, service_name=None, sid=None):
    ...
    cn = _dbq_open(dbtype, host, port, user, pw, dbname, service_name, sid)
    with _DBQ_LOCK:
...
def _dbquery(arg):
    ...
    service_name = req.get("service_name") or None
    sid = req.get("sid") or None
    sql = req.get("sql") or ""
    key = (dbtype, host, port, user, dbname, service_name, sid)
    ...
    try:
        cn, reused = _dbq_get_conn(key, dbtype, host, port, user, pw, dbname, service_name, sid)
    except Exception as e:  # noqa: BLE001
    ...
    try:
        ...
        if reused:
            _dbq_drop(key)
            try:
                cn, _ = _dbq_get_conn(key, dbtype, host, port, user, pw, dbname, service_name, sid)
                return _run(cn)
...
_CROSS_PLATFORM_OPS = ("dbquery",)   # ops safe to run on the Linux Python agent
