...
def _agent_dbquery(token, conn, sql):
    ...
    payload = json.dumps({
        ...
        # Oracle identifies the target via service_name OR sid, not database_name —
        # omitting these made every agent-routed Oracle connection fall back to the
        # default service "XE" regardless of the configured target (see _dbq_open).
        "service_name": getattr(conn, "service_name", None) or "",
        "sid": getattr(conn, "sid", None) or "",
        "sql": sql,
    })
