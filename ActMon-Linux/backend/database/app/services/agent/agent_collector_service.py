...
def _run_snaps(agent_name, conn_id, db, snaps):
    """Build snapshots; if agent-routed, only the first (main dashboard) to avoid
    flooding the agent's job channel."""
    if _is_agent_routed(conn_id, db):
        snaps = snaps[:1]
    ...
    try:
        ...
        for agent in agents:
            try:
                ...
                # At a 15s cadence a single queued/slow cycle is normal, not an
                # outage — flipping status per-cycle made agents flap online↔error
                # continuously. Only show "error" after ERROR_STREAK consecutive
                # failures (~45s of real trouble); one success clears it at once.
                if success:
                ...
                if success:
                    # Metrics run every cycle (15s); heavy dashboard SNAPSHOT builds
                    # (~20-50 queries each) stay throttled to once per minute so a
                    # fast metrics cadence never floods the agent job channel.
