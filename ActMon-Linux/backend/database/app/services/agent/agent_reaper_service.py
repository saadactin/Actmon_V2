...
# 3 min = 12 missed pushes at the 15s cadence — a truly dead agent, not one that's
# merely busy with a long job (120s made those flap offline↔online). Env-overridable.
OFFLINE_AFTER = int(os.getenv("AGENT_OFFLINE_SECS", "180"))
REMOVE_AFTER = int(os.getenv("AGENT_REMOVE_SECS", "900"))      # 15 min silence -> uninstalled

# PostgreSQL metrics retention: at the 15s cadence agent_metrics grows fast; keep a
# rolling window here (ClickHouse holds long history when enabled). Pruned hourly.
...
def _prune_metrics(db):
    """Hourly: trim fast-growing telemetry tables to the retention window. The 15s
    pipeline writes ~6k rows/agent/day into agent_metrics — without this, PostgreSQL
    bloats and dashboard queries slow down over weeks."""
    global _last_prune
    import time as _t
    if (_t.monotonic() - _last_prune) < 3600 and _last_prune:
        return
    ...
    for table, col in (("agent_metrics", "timestamp"), ("agent_top_sql", "timestamp"),
        ...
        try:
            r = db.execute(text(
                f"DELETE FROM {table} WHERE {col} < now() - make_interval(days => :d)"),
                {"d": METRICS_RETENTION_DAYS})
            if r.rowcount:
                logger.info("[reaper] pruned %s rows from %s (>%sd)",
                            r.rowcount, table, METRICS_RETENTION_DAYS)
...
def reap_once():
    ...
    try:
        _prune_metrics(db)
        # 1) Stale agents -> offline. Capture which ones flip so we alert exactly once.
        going_offline = [row[0] for row in db.execute(text(
            "SELECT agent_name FROM agents "
            "WHERE status <> 'offline' AND last_heartbeat IS NOT NULL "
            "AND last_heartbeat < now() - make_interval(secs => :s)"), {"s": OFFLINE_AFTER}).fetchall()]
        r = db.execute(text(
