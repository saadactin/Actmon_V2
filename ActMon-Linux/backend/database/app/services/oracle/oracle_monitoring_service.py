...
def oracle_dashboard(conn_id: int, db: Session):
    ...
    # Diagnostic only — lets an admin tell at a glance whether this dashboard is
    # querying through the host agent (dbquery) or a direct connection from the
    # ActMon server, since the failure mode differs by path (see db_proxy_service).
    try:
        from app.services.common.db_proxy_service import agent_host_for_conn
        collector = "agent" if agent_host_for_conn(conn_id, db) else "direct"
    except Exception:
        collector = "unknown"
    ...
    # Every metric above degrades to 0/"" on its own query failure rather than
    # raising, so "status": "success" alone can't be trusted — a completely dead
    # connection (bad service_name/SID, agent unreachable, missing grants) looks
    # identical to a healthy-but-empty one. Surface that explicitly: if the core
    # identity queries (version/instance/database) ALL failed, nothing else could
    # have possibly worked either.
    _core_failed = all(
        any(e.startswith(p) for e in errors) for p in ("version:", "instance:", "database:")
    )

    _res = {
        "status": "success",
        "degraded": _core_failed,
        "collector": collector,
        "connection": {
            "id":           conn.id,
