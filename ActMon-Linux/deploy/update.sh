...
if [ -f "$BUNDLE/deploy/nginx-actmon.conf" ]; then
  ...
  # redirects), preserving the port/host already in use.
  # RHEL/Oracle Linux nginx has no sites-available convention — install.sh drops the
  # site straight into conf.d/. Debian/Ubuntu boxes use sites-available + a symlink
  # in sites-enabled (a stale standalone copy there would silently shadow every edit,
  # so always re-symlink rather than writing sites-enabled directly).
  if [ -f /etc/nginx/conf.d/actmon.conf ]; then
    CUR="/etc/nginx/conf.d/actmon.conf"; TARGET="$CUR"
  else
    CUR="/etc/nginx/sites-enabled/actmon"; [ -f "$CUR" ] || CUR="/etc/nginx/sites-available/actmon"
    TARGET="/etc/nginx/sites-available/actmon"
  fi
  ...
  sed "s#__APP_DIR__#$APP_DIR#g; s#__HOST__#$HOST#g; s#__WEB_PORT__#$WEB_PORT#g" \
      "$BUNDLE/deploy/nginx-actmon.conf" > "$TARGET"
  [ "$TARGET" = "/etc/nginx/sites-available/actmon" ] && ln -sf "$TARGET" /etc/nginx/sites-enabled/actmon
