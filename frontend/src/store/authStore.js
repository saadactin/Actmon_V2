...
export const useAuthStore = create((set, get) => {
  ...
  return {
    ...
    setToken: (token, user) => {
      ...
      if (user) localStorage.setItem('actmon_user', JSON.stringify(user));
      set({ token, user });
      setupAutoLogout(token);
    },
    setUser: (user) => {
      localStorage.setItem('actmon_user', JSON.stringify(user));
      set({ user });
    },
    ...
    setAuth: (token, payload = {}) => {
      ...
      localStorage.setItem('actmon_token', token);
      localStorage.setItem('actmon_user', JSON.stringify(mergedUser));
