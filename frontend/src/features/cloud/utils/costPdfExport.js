...
export async function downloadCostReportPdf(filename, title, subtitle, columns, rows) {
  const { jsPDF } = await import('jspdf');
  const pdf = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4' });

  const pageW = 297; const pageH = 210;
  const marginX = 10; const marginTop = 15; const marginBottom = 10;
  const rowHeight = 6; const headerHeight = 7;
