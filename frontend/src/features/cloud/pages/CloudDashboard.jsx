...
import {
  Cloud, Server, DollarSign, Activity, Shield, ShieldAlert, ShieldCheck, ArrowRight, ArrowLeft,
  Clock, Loader2, Layers, Globe, Sparkles, Plus,
} from 'lucide-react';
import {
...
const PROVIDER_ICON_CHIP_CLASSES = {
...
export const CloudDashboard = () => {
  ...
  const [selectedViewState, setSelectedView] = useState('ALL');
  ...
  const isAccountSelected = (accId) => (selectedView === 'ALL' ? visibleAccountIds.has(accId) : accId === selectedView);
  ...
  const billedCosts = relevantCostQueries
    .map(({ query }) => query?.data)
    .filter((c) => !!c && typeof c.total_monthly_cost === 'number');
