...
export const CloudShell = () => {
  ...
  const resourceCount = isAccountScoped
    ? (resources || []).filter((r) => r.account_id === routeAccountId).length
    : scope.filterByScope(resources).length;
  const providers = Array.from(
    new Set((scope.isScoped ? scope.scopedAccounts : accounts || []).map((a) => providerKeyOf(a.provider))),
  );
