...
import { buildPuppetManifest, puppetRunCommand } from '../puppetManifest';
...
// Real "Instruction" step for the Puppet automated-deployment method — a
// generated manifest that downloads and runs ActMon's own existing
// actmon-setup.sh/.ps1 on the target host, same install path the manual
// "Script-based installation" method uses (see puppetManifest.js). `puppet
// apply` (used here) is the one Puppet mode that needs no Puppet Server at
// all, so the instructions apply the manifest locally on each target.
export default function StepInstructionPuppet({ data, setData }) {
  ...
  const manifest = data.token ? buildPuppetManifest({ token: data.token, apiBase, targetOs }) : '';
  const runCmd = puppetRunCommand(targetOs);
  const manifestFile = `deploy-actmon-agent-${targetOs}.pp`;
  ...
  return (
    <div className="max-w-4xl">
      <h3 className="text-[19px] font-black text-slate-800">Deploy with Puppet</h3>
      <p className="text-[15px] text-slate-600 mt-2 leading-relaxed">
        This generates a manifest that installs the ActMon Agent — it downloads and runs the same
        install script the manual method uses. No agent-install logic is reimplemented in the manifest.
      </p>
      ...
      <div className="mt-5 rounded-lg bg-red-50 border border-red-200 p-3.5 flex items-start gap-3">
        ...
        <p className="text-[13px] text-slate-700 leading-relaxed">
          <b>The generated manifest contains your ActMon registration token.</b> Treat it as a secret: do
      ...
      <h4 className="mt-6 text-[15px] font-black text-slate-800">1. Manifest</h4>
      {manifest ? (
        <>
          <div className="mt-1 flex items-center justify-between gap-3">
            <p className="text-[13px] text-slate-500">Save this as <span className="font-mono">{manifestFile}</span>.</p>
            <button
              onClick={() => dl(manifestFile, manifest)}
          ...
          <CodeBox text={manifest} onCopy={() => copy('manifest', manifest)} copied={copiedWhich === 'manifest'} />

          <h4 className="mt-6 text-[15px] font-black text-slate-800">2. Run it (on each target host)</h4>
        ...
        <div className="mt-2 rounded-lg bg-amber-50 border border-amber-200 p-3.5 flex items-start gap-3">
          ...
          <p className="text-[13px] text-slate-700 leading-relaxed">
            Couldn't generate a manifest — the registration token or the detected server URL didn't pass
      ...
      <div className="mt-5 rounded-lg bg-blue-50 border border-blue-200 p-3.5 flex items-start gap-3">
        ...
        <p className="text-[13px] text-slate-700 leading-relaxed">
          Puppet normally runs pulled by a puppet agent from a Puppet Server, unlike Ansible's push
          model — <span className="font-mono">puppet apply</span> (used above) needs no server at all,
          so copy the manifest to each target host and run it there. If you have a Puppet Server/Bolt
          set up, add this manifest to your module instead. Each host appears under <b>Agents</b> as
          soon as it checks in.
        </p>
      </div>
    </div>
  );
