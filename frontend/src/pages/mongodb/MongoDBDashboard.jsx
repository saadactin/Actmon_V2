...
import { adaptMongoCollectionDetails } from '@/pages/_shared/tableDetailsAdapters';
...
import Pagination, { Paged } from '@/components/ui/Pagination';
...
/* DetailModal / TypeBadge / CollectionDetailPanel removed — collection
   inspection now goes through the shared <TableDetailsDialog>, the same
   component every engine dashboard uses. */
...
export default function MongoDBDashboard() {
  ...
  // Collection details — the shared ActMon Table Details UI.
  const {
    data: collDetailData, isLoading: collDetailLoading,
    isError: collDetailError, refetch: refetchCollDetail,
  } = useQuery({
    queryKey: ['mongoCollDetail', id, selectedColl?.dbName, selectedColl?.name],
    queryFn: () => client.get(`/connections/mongodb/${id}/mongo-collection-detail/${selectedColl.dbName}/${selectedColl.name}`).then((r) => r.data),
  ...
  if (isLoading) return (
    <PageLoading title="Connecting to MongoDB…" subtitle="Fetching live metrics" />
  );
  ...
  return (
    ...
    <div className="flex min-h-full flex-col">
      ...
      <TableDetailsDialog
        ...
        breadcrumb={selectedColl ? { database: selectedColl.dbName, schema: null, table: selectedColl.name } : null}
        isLoading={collDetailLoading}
        isError={collDetailError || collDetailData?.status === 'error'}
        onRetry={refetchCollDetail}
        data={adaptMongoCollectionDetails(collDetailData)}
      />

      {/* Header, tab strip, quick links, health and the refresh countdown all come
          from the shared EngineDashboardHeader — same component and the same
          configurable surface as every other engine's dashboard. The 🍃 mark and
          MongoDB's accent colour come from config/engines.js. */}
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                ...
                <Panel title={`Databases (${databases.length})`}>
                  <div className="overflow-x-auto">
                    <Paged rows={databases} unit="databases">{(pageRows, pager) => (<>
          ...
          return (
            <div className="space-y-5">
              ...
              <Panel title={`Current Operations (${ops.length})`}
                ...
                <div className="overflow-x-auto">
                  <Paged rows={ops} unit="operations">{(pageRows, pager) => (<>
          ...
          return (
            <div className="space-y-5">
              ...
              <Panel title={`Profiled Operations (${profilerOps.length})`}>
                <div className="overflow-x-auto">
                  <Paged rows={profilerOps} unit="operations">{(pageRows, pager) => (<>
                  <table className="w-full text-xs">
                    <thead className="bg-slate-50">
          ...
          return (
            <div className="space-y-4">
              ...
              <Panel title="Collection Statistics">
                <div className="overflow-x-auto">
                  <Paged rows={filtered} unit="collections">{(pageRows, pager) => (<>
                  <table className="w-full text-xs">
                    <thead className="bg-slate-50">
            ...
            <div className="space-y-5">
              ...
              {(og.recent_entries || []).length > 0 && (
                <Panel title={`Recent Oplog Entries (last ${og.recent_entries.length})`}>
                  <div className="overflow-x-auto">
                    <Paged rows={og.recent_entries} unit="entries">{(pageRows, pager) => (<>
                    <table className="w-full text-xs">
                      <thead className="bg-slate-50">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <Panel title={`Shards (${(sh.shards || []).length})`}>
                  <div className="overflow-x-auto">
                    <Paged rows={sh.shards || []} unit="shards">{(pageRows, pager) => (<>
                    <table className="w-full text-xs">
                      <thead className="bg-slate-50">
            ...
            <div className="space-y-4">
              ...
              {Object.entries(dbGroups).sort(([a],[b]) => a.localeCompare(b)).map(([dbName, dbUsers]) => (
                <Panel key={dbName} title={`${dbName} (${dbUsers.length} user${dbUsers.length !== 1 ? 's' : ''})`}
                  ...
                  <div className="overflow-x-auto">
                    <Paged rows={dbUsers} unit="users">{(pageRows, pager) => (<>
                    <table className="w-full text-xs">
                      <thead className="bg-slate-50">
                        <tr>
