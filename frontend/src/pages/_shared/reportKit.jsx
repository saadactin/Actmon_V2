 ...
 *
 * MySQL reports "8.0.41", but PostgreSQL's version() is a sentence — "PostgreSQL
 * 17.9 on x86_64-windows, compiled by msvc-19.44.35225, 64-bit". Put that in a
 * header subtitle and it pushes the action buttons onto their own line, so the two
 * reports stop looking alike. One helper, so every engine reads the same.
 */
export function shortVersion(v) {
  if (!v) return null;
  const m = String(v).match(/\d+(?:\.\d+)*/);
  return m ? m[0] : String(v).split(/[\s,(]/)[0] || null;
}
...
export function ReportShell({
  ...
  return (
    <div className="min-h-full" id={`${engine.key}-report-root`}>
      {showEmailModal && (
      ...
      {/* ─── CONTROL HEADER ───
          The shared PageHeader, not a bespoke gradient band: reports then carry
          the same configurable header surface (colour / gradient / texture) and the
          same pinning as every other page, and a new engine's report inherits it
          with no work. `no-print` keeps the controls out of the printed document.
          Controls are token-styled so they stay legible whatever surface the
          operator configures — the header remaps --fg/--border on its subtree. */}
      <PageHeader
        className="no-print"
        title={`${engine.label} Monitoring Report`}
        subtitle={`${connName}${subtitle ? ` — ${subtitle}` : ''}`}
        backTo={engine.backTo}
        leading={(
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-md bg-accent-soft text-[20px]">
            {engine.emoji}
          </span>
        )}
        actions={(
          <div className="flex flex-wrap items-center justify-end gap-2">
            <div className="flex items-center gap-0.5 rounded-control border border-border p-0.5">
              {PERIODS.map(p => (
                <button key={p.id} onClick={() => setPeriod(p.id)}
                  ...
                  className={`h-7 rounded-control px-2.5 text-[11px] font-bold transition-colors ${
                    period === p.id ? 'bg-accent text-accent-fg' : 'text-muted hover:bg-sunken hover:text-fg'}`}>
                  {p.label}
                </button>
              ))}
            </div>
            <button onClick={() => { onRefresh?.(); setGenTime(now()); }}
              className="flex h-8 items-center gap-1.5 rounded-control border border-border px-3 text-[12px] font-semibold text-muted transition-colors hover:bg-sunken hover:text-fg">
              <RefreshCw size={13} /> Refresh
            </button>
            {canHere('execute') && (
              <button onClick={openEmailModal} disabled={capturingPdf}
                className="flex h-8 items-center gap-1.5 rounded-control border border-border px-3 text-[12px] font-semibold text-fg transition-colors hover:bg-sunken disabled:opacity-60">
                {capturingPdf
            ...
            {canHere('export') && (
              <button onClick={handlePrint}
                className="flex h-8 items-center gap-1.5 rounded-control bg-accent px-3 text-[12px] font-bold text-accent-fg transition-colors hover:bg-accent-hover">
      ...
      >
        <div className="mt-gutter-sm flex flex-wrap items-center gap-3">
          <span className="rounded-full px-3 py-1 text-[11px] font-bold"
            style={{ background: periodObj.color + '22', color: periodObj.color, border: `1px solid ${periodObj.color}55` }}>
            {periodObj.label} Report
          </span>
          <span className="text-[11px] text-subtle">Generated: {genTime}</span>
          {period === 'live' && (
            <span className="flex items-center gap-1.5 text-[10px] font-bold text-success-fg">
              <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-success" />AUTO-REFRESH 30s
            </span>
          )}
          {alertBadges.filter(Boolean).map((b, i) => (
            <span key={i} className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${
              b.tone === 'red' ? 'bg-danger-soft text-danger-fg' : 'bg-warning-soft text-warning-fg'}`}>
