...
import {
  DATABASE_COLUMNS, PG_DATABASE_EXTRAS, PG_DATABASE_HINTS, withExtras, withHints,
...
/** Health pill for the shared engine header. Same thresholds as every engine. */
...
export default function PostgreSQLDashboard() {
  ...
  /* ─── loading ─── */
  if (isLoading) return (
    <PageLoading title="Connecting to PostgreSQL…" subtitle="Fetching live metrics" />
  );
  ...
  return (
    ...
    <div className="flex min-h-full flex-col">
      ...
      {/* Header, tab strip, quick links, health and the refresh countdown all come
          from the shared EngineDashboardHeader — same component, same geometry and
          the same configurable surface as every other engine's dashboard. The
          engine's mark and colour come from config/engines.js. */}
      ...
      <div className="flex-1">
        ...
        {/* ══ DATABASES ══ */}
        {activeTab === 'databases' && (() => {
          /* Same shared list panel as every other engine's Databases tab — the
             eight common columns come from config/dbCatalog, and PostgreSQL's
             extras (live connections, commits/rollbacks) are spliced in rather
             than forking the column set. Sorting, search, the card/table switch
             and paging all come with the panel. */
          const byDb = {};
          (tablesDetail?.tables || []).forEach((t) => {
            const k = t.database || '';
            (byDb[k] = byDb[k] || { count: 0, bytes: 0 });
            byDb[k].count += 1;
            byDb[k].bytes += Number(t.total_bytes || 0);
          });
          /* The table count is a roll-up of the tables endpoint, not something
             pg_stat_database reports, so it is merged in here. */
          const items = databases.map((db) => ({
            ...db,
            tables: byDb[db.name]?.count ?? null,
          }));
          const openDbTables = (db) => { setTablesDb(db.name); setActiveTab('tables'); };
          return (
            <ObjectTable
              title="Databases"
              icon="database"
              columns={withHints(withExtras(DATABASE_COLUMNS, PG_DATABASE_EXTRAS), PG_DATABASE_HINTS)}
              items={items}
              loading={tablesLoading && !databases.length}
              searchOn={['name', 'owner', 'encoding', 'collation']}
              searchPlaceholder="Search databases…"
              defaultSort={{ key: 'size', dir: 'desc' }}
              note="Click a database to see its tables"
              unit="databases"
              onOpen={openDbTables}
          ...
          <div className="space-y-5">
            ...
            <Panel title={`pg_locks (${pg_locks.length})`} icon={<Lock size={15} className="text-slate-400"/>}>
              <div className="overflow-x-auto">
                <Paged rows={pg_locks} unit="locks">{(pageRows, pager) => (<>
    ...
    <div className="space-y-5">
      ...
      {senders.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <Paged rows={senders} unit="replicas">{(pageRows, pager) => (<>
      ...
      {replicas.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <Paged rows={replicas} unit="replicas">{(pageRows, pager) => (<>
      ...
      {isStandby && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {confls.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <Paged rows={confls} unit="conflicts">{(pageRows, pager) => (<>
              <table className="w-full text-[12px]">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/50">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <Paged rows={active} unit="queries">{(pageRows, pager) => (<>
          <table className="w-full text-[12px]">
            <thead>
              <tr className="bg-slate-50/70 border-b border-slate-200">
...
/* TableDeepDiveModal / TableDetailPane removed — table inspection now goes
...
function AdvancedTablesTab({ detail, isLoading, refetch, connId, initialDb }) {
  ...
  const {
    data: tableDetail, isLoading: tableDetailLoading,
    isError: tableDetailError, refetch: refetchTableDetail,
  } = useQuery({
    queryKey: ['pgTableDetail', connId, selectedTable?.database, selectedTable?.schemaname, selectedTable?.relname],
    queryFn: () => client.get(`/connections/postgresql/${connId}/table-structure`, {
      params: { database: selectedTable.database, schema: selectedTable.schemaname, table: selectedTable.relname },
    }).then((r) => r.data),
  ...
  /* 110 tables in one scroll is not a list anyone reads. Same stepper, same
     ...
     list — the hook exists so a hand-written table gets it without a rewrite. */
  const paged = usePaged(tables);
  ...
  return (
    <div className="space-y-5">
      ...
      <TableDetailsDialog
        ...
        breadcrumb={selectedTable ? { database: selectedTable.database, schema: selectedTable.schemaname, table: selectedTable.relname } : null}
        isLoading={tableDetailLoading}
        isError={tableDetailError || tableDetail?.status === 'error'}
        errorMessage={tableDetail?.error}
        onRetry={refetchTableDetail}
        data={adaptPostgresTableDetails(tableDetail)}
      />
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-indigo-50 to-violet-50/30 border-b border-slate-100 px-5 py-3">
          ...
          <div className="flex gap-1.5 mt-3 flex-wrap">
            {TABLE_VIEWS.map(tv => (
              <button key={tv.id} onClick={()=>setView(tv.id)}
        ...
        {tables.length === 0 && !isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[12px]">
              <thead>
                <tr className="bg-slate-50/70 border-b border-slate-200">
                  {['DB','Schema','Table','Total Size','Heap','Indexes','Live Rows','Dead Rows','Dead %','Cache Hit','Seq Scans','Idx %','Ins/Upd/Del','Last Vacuum','Last Analyze'].map(h=>(
                    <th key={h} className="px-3 py-2.5 text-left text-[9px] font-black text-slate-400 uppercase tracking-wide whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {paged.rows.map((t, i) => {
                  ...
                  return (
                      <tr key={i} className={`border-t border-slate-100 hover:bg-indigo-50/20 transition-colors ${needsVac?'bg-orange-50/30':''}`}>
                        <td className="px-3 py-2.5">
            ...
            <Pagination {...paged.props} unit="tables" />
          </div>
        )}
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-4 items-start">
        ...
        <div className="flex-1 min-w-0 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <Paged rows={params} unit="parameters">{(pageRows, pager) => (<>
            <table className="w-full text-[12px]">
              <thead>
                <tr className="bg-slate-50/70 border-b border-slate-200">
                  {[
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {activeSection === 'roles' && (
          <div className="overflow-x-auto">
            <Paged rows={roles} unit="roles">{(pageRows, pager) => (<>
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
        ...
        {activeSection === 'activity' && (
          <div className="overflow-x-auto">
            <Paged rows={activity} unit="sessions">{(pageRows, pager) => (<>
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
        ...
        {activeSection === 'members' && (
          <div className="overflow-x-auto">
            {members.length === 0 ? (
              ...
              <Paged rows={members} unit="members">{(pageRows, pager) => (<>
              <table className="w-full text-[12px]">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/50">
        ...
        {activeSection === 'objects' && (
          <div className="overflow-x-auto">
            {ownership.length === 0 ? (
              ...
              <Paged rows={ownership} unit="objects">{(pageRows, pager) => (<>
              <table className="w-full text-[12px]">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/50">
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {activeSection === 'tables' && (
          <div className="overflow-x-auto">
            <Paged rows={topTables} unit="tables">{(pageRows, pager) => (<>
            <table className="w-full text-[11px]">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
        ...
        {activeSection === 'bloat' && (
          <div className="overflow-x-auto">
            {bloatTables.length === 0 ? (
              ...
              <Paged rows={bloatTables} unit="tables">{(pageRows, pager) => (<>
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/50">
        ...
        {activeSection === 'vacuum' && (
          <div className="overflow-x-auto">
            {vacuumNeeded.length === 0 ? (
              ...
              <Paged rows={vacuumNeeded} unit="tables">{(pageRows, pager) => (<>
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/50">
        ...
        {activeSection === 'spaces' && (
          <div className="overflow-x-auto">
            <Paged rows={tablespaces} unit="tablespaces">{(pageRows, pager) => (<>
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
      ...
      {toastTables.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <Paged rows={toastTables} unit="tables">{(pageRows, pager) => (<>
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
