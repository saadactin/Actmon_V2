...
import {
  ...
  ChevronDown, ChevronUp, FolderOpen, Key,
...
export default function MySQLDashboard() {
  ...
  // Sort state for the tables list. Shared by the sort-preset buttons and the
  // sortable column headers, so both drive one source of truth.
  const [tblSort, setTblSort]   = useState(TABLE_SORT_PRESETS[0].sort);
  ...
  const { data: dbListData, isLoading: dbListLoading } = useQuery({
    ...
    retry: false,
    refetchInterval: 30000,
    // The Databases tab renders this list in full, so it has to be fetched there
    // too — not only alongside the tables explorer.
  ...
  const { data: tblDetailData, isLoading: tblDetailLoading, isError: tblDetailError, refetch: refetchTblDetail } = useQuery({
  ...
  // Fetched eagerly alongside the detail query (not gated to the Sample Data
  // tab) — the shared TableDetailsDialog has one loading state for the whole
  // dialog, not per-tab, and a LIMIT-100 sample is cheap either way.
  const { data: tblSampleData, isLoading: tblSampleLoading, isError: tblSampleError } = useQuery({
  ...
  return (
    ...
    <div className="flex min-h-full flex-col">
      ...
      {/* ─── HEADER ───
          The shared engine header: same on every technology dashboard, with the
          engine's mark/accent and its sub-page links coming from config/engines.js. */}
      ...
      {/* A render crash in one tab (e.g. a chart fed a transient bad value)
          used to blank the ENTIRE page via the app-shell's ErrorBoundary,
          taking the tab strip/health badge/refresh control down with it.
          Scoping a boundary here means only this tab's body is replaced —
      ...
      <ErrorBoundary resetKey={activeTab}>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
              ...
              <Panel title="Server Information">
                <Row label="Hostname"       value={health_summary.host_name || server_vars.hostname || '—'} mono
                  to={connection.server_id ? `/infra/${connection.server_id}` : undefined} />
          ...
          <div className="space-y-5">
            ...
            <Panel title={`Full Process List (${process_list.length})`}>
              <div className="overflow-x-auto">
                <Paged rows={process_list} unit="processes">{(pageRows, pager) => (<>
        ...
        {/* ══ DATABASES ═════════════════════════════════════════════
            Full per-schema metadata comes from /connections/mysql/:id/databases;
            the overview payload's `databases` carries only name/count/size (it
            feeds the charts), so this tab reads the detailed endpoint and falls
            back to the overview list while that first request is in flight. */}
        {activeTab === 'databases' && (() => {
          const detailed = dbListData?.data || [];
          const rows = detailed.length ? detailed : databases;
          const openTables = (d) => { setSelDb(d.name); setTblDbFilter(d.name); setActiveTab('tables'); };
          return (
            <ObjectTable
              title="Databases"
              icon="database"
              ...
              items={rows}
              loading={dbListLoading && !detailed.length}
              searchOn={['name', 'owner', 'charset', 'collation']}
              searchPlaceholder="Search databases…"
              unit="databases"
              defaultSort={{ key: 'size', dir: 'desc' }}
              note="Click a row to browse its tables"
              onOpen={openTables}
              onRowClick={openTables}
              emptyTitle="No databases found"
              emptyBody="This server has no user schemas, or the monitoring user cannot see them."
            />
          );
        })()}
        ...
        {activeTab === 'tables' && (() => {
          ...
          const scoped   = all.filter(t => tblDbFilter === '__all__' || t.database === tblDbFilter);
          const largest  = [...scoped].sort((a, b) => (b.total_bytes || 0) - (a.total_bytes || 0))[0];
          const openTable = (t) => { setSelDb(t.database); setSelTable(t.name); };
          const kpis = [
            ...
            { label: 'Largest Table',value: largest?.name || '—',           color: 'emerald' },
          ...
          <div className="space-y-5">
            ...
            {/* Tables. Columns, formatting and sorting all come from
                config/dbCatalog, so this list and the Databases list are the same
                component with a different column set. The sort presets set the
                same state a header click does, so the two never disagree. */}
            <ObjectTable
              title="Tables"
              icon="table"
              ...
              filter={allDbs.length > 1 ? {
                value: tblDbFilter,
                onChange: setTblDbFilter,
                options: [
                  { id: '__all__', label: `All databases (${totalCnt})` },
                  ...allDbs.map((db) => ({
                    id: db,
                    label: `${db} (${all.filter((t) => t.database === db).length})`,
                  })),
                ],
              } : undefined}
              actions={
                <IconButton
                  ...
                  size="sm"
                  onClick={refetchAllTables}
                  iconClassName={allTablesLoading ? 'animate-spin' : undefined}
                />
              }
              toolbar={TABLE_SORT_PRESETS.map((p) => {
                const on = tblSort?.key === p.sort.key && tblSort?.dir === p.sort.dir;
                return (
                  <Button
                    key={p.id}
                    size="sm"
                    variant={on ? 'subtle' : 'ghost'}
                    onClick={() => setTblSort(p.sort)}
                  >
                    {p.label}
                  </Button>
                );
              ...
              emptyTitle={totalCnt === 0 ? 'No user tables found' : 'No tables match the filter'}
              emptyBody={totalCnt === 0
                ? 'This server has no base tables outside the system schemas.'
                : undefined}
            />

            {/* ── Table details — the shared ActMon Table Details UI, the
                same component every engine dashboard opens on "Inspect" ── */}
            <TableDetailsDialog
              open={!!selTable}
              onClose={() => setSelTable(null)}
              breadcrumb={selTable ? { database: selDb, schema: null, table: selTable } : null}
              isLoading={tblDetailLoading || tblSampleLoading}
              isError={tblDetailError || tblSampleError || tblDetailData?.status === 'error'}
              errorMessage={tblDetailData?.error}
              onRetry={refetchTblDetail}
              data={adaptMysqlTableDetails(tblDetailData, tblSampleData)}
            />
          </div>
          );
        })()}
                ...
                <>
                  ...
                  <Panel title={`Active Transactions (${txns.length})`}
                    ...
                    {txns.length === 0 ? (
                      ...
                      <div className="overflow-x-auto">
                        <Paged rows={txns} unit="transactions">{(pageRows, pager) => (<>
                        <table className="w-full text-sm">
                          <thead className="bg-slate-50">
            ...
            return (
              <div className="space-y-5">
                ...
                <Panel title="User Sessions">
                  <div className="overflow-x-auto">
                    <Paged rows={users} unit="users">{(pageRows, pager) => (<>
                    <table className="w-full text-sm">
                      <thead className="bg-slate-50">
...
function Row({ label, value, mono, to }) {
  const valueCls = `text-right font-semibold text-xs ${mono ? 'font-mono' : ''} break-all`;
  return (
    <div className="flex items-start justify-between gap-4 py-1.5 border-b border-slate-100 last:border-0">
      ...
      {to ? (
        <Link to={to} className={`${valueCls} text-blue-600 hover:text-blue-800 hover:underline`} title="Open this host's Infrastructure page">
          {value}
        </Link>
      ) : (
        <span className={`${valueCls} text-slate-800`}>{value}</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex" style={{ backdropFilter: 'blur(2px)', background: 'rgba(0,0,0,0.45)' }}
      ...
      <div className="ml-auto h-full flex flex-col bg-white shadow-2xl overflow-hidden"
        ...
        {/* Themed via the shared brand gradient token rather than fixed cyan/teal,
            so the drill-down panel follows the accent like everything else. */}
        <div className="flex flex-shrink-0 items-start justify-between px-6 py-4 text-accent-fg"
          style={{ background: 'var(--gradient-accent)' }}>
          <div className="min-w-0">
            <h2 className="truncate text-lg font-black">{title}</h2>
            {subtitle && <p className="mt-0.5 text-xs text-accent-fg/75">{subtitle}</p>}
          </div>
