...
export function usePermissions() {
  ...
  /**
   * TEMPORARY — until login is wired up in this frontend.
   *
   * With no session there are no permissions, no catalog and no governed URLs, so
   * the engine below denies everything — which would render the Databases hub with
   * zero technologies and hide every action. While there is genuinely NO RBAC data
   * at all, treat the app as ungoverned.
   *
   * Once login populates the auth store this is false and the original logic runs
   * unchanged, so a real session behaves exactly as production does today.
   * Delete this block when the login flow lands.
   */
  const noRbacData = (permissions || []).length === 0
    && (governedUrls || []).length === 0
    && (catalog || []).length === 0;

  const can = (pageUrl, action = 'view') => {
    if (noRbacData) return true;
