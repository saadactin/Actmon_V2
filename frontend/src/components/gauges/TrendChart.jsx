...
export default function TrendChart({
  ...
  const requestedType = styleOverride || chartStyle;
  // Bubble needs three independent numeric dimensions (X, Y, size) — a
  // genuine dataset, never a chronological trend, which only ever has one
  // value per moment in time. When the operator's global "chart style"
  // setting is Bubble, every plain time-series sparkline in the app (xKey
  // still defaulted to 't') would otherwise get routed through bubble's X
  // axis, which does `Number(timeString)` → NaN → a degenerate axis domain
  // that intermittently threw "[DecimalError] Invalid argument: NaN" out of
  // recharts-scale during the container's first real resize. Falling back to
  // 'area' here is the same thing BUILT_STYLES already does for styles that
  // flat-out don't exist — this is a style that exists but doesn't apply.
  const type = requestedType === 'bubble' && xKey === 't' ? 'area' : requestedType;
  ...
  return (
    <div>
      ...
      {/* `debounce` smooths over the very first ResizeObserver callback firing
          with a transient near-zero container width (common right after a tab
          mounts, before CSS layout settles) — undebounced, recharts computes
          an axis scale against that bogus width and recharts-scale's
          decimal.js-light throws "[DecimalError] Invalid argument: NaN" on
          the degenerate domain, crashing the whole chart instead of just
          rendering once more a moment later with the real size. */}
