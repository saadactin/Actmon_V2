...
export const INDICATOR_STYLES = ['ring', 'stat', 'donut', 'minimal', 'dial'];
...
export function DashboardAppearanceProvider({ children }) {
  ...
  useEffect(() => {
    /* These styles are stored per user, so there is nothing to fetch while signed
       out — and asking anyway produced a steady stream of 401s in the server log
       (two per mount) that told the operator nothing. Defaults are used instead. */
    let token = null;
