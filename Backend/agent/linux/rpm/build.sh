...
[ -f "$COMMON/actmon-agent.sh" ]      || { echo "agent script not found in $COMMON"; exit 1; }
...
cp "$COMMON/actmon-agent.sh"       "$TOP/SOURCES/actmon-agent.sh"
cp "$COMMON/actmon-agent.service"  "$TOP/SOURCES/actmon-agent.service"
cp "$HERE/actmon-agent.spec"       "$TOP/SPECS/actmon-agent.spec"
sed -i 's/\r$//' "$TOP/SOURCES/actmon-agent.sh" "$TOP/SOURCES/actmon-agent.service" "$TOP/SPECS/actmon-agent.spec"
