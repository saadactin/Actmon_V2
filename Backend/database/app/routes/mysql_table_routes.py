...
====================
Advanced table introspection using INFORMATION_SCHEMA and SHOW statements.
No special privileges beyond SELECT on information_schema are required.

Endpoints
---------
GET /{id}/databases                              — list all databases
GET /{id}/databases/{db}/tables                  — list tables + metadata
GET /{id}/databases/{db}/tables/{table}          — full table detail
GET /{id}/databases/{db}/tables/{table}/data     — sample rows (LIMIT 50)
GET /{id}/databases/{db}/tables/{table}/ddl      — SHOW CREATE TABLE
"""
    ...
    try:
        st_rows = _rows(engine, """
            SELECT
                ...
                TABLE_COLLATION, ROW_FORMAT, CREATE_TIME, UPDATE_TIME,
                CHECK_TIME, CREATE_OPTIONS, TABLE_COMMENT
...
def table_sample_data(conn_id: int, db_name: str, table_name: str,
                      db: Session = Depends(get_db)):
    conn = _get_conn(conn_id, db)
    try:
        engine = _engine(conn)

        # column list first (for ordering & type hints)
        cols = _rows(engine, """
