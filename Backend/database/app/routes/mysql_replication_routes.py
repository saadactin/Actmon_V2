"""
MySQL / MariaDB Replication & Galera Cluster Monitor
=====================================================
Covers:
  • Traditional async replication  — SHOW SLAVE STATUS / SHOW REPLICA STATUS
  • Galera / wsrep cluster metrics  — SHOW GLOBAL STATUS LIKE 'wsrep%'
  • Binary log master status        — SHOW MASTER STATUS
  • Key replication variables       — server_id, log_bin, binlog_format, …
  • GTID state                      — @@gtid_binlog_pos / @@gtid_slave_pos

Endpoints
---------
GET /{id}/replication/status      — full snapshot (master + slave + galera)
GET /{id}/replication/variables   — all replication-related system variables
"""
...
def replication_status(conn_id: int, db: Session = Depends(get_db)):
    ...
    if slave_raw:
        ...
        last_err    = _v("Last_Error", "Last_Errno") or ""
        last_io_err = _v("Last_IO_Error", "Last_IO_Errno") or ""
        last_sql_err= _v("Last_SQL_Error","Last_SQL_Errno") or ""
...
def replication_variables(conn_id: int, db: Session = Depends(get_db)):
    ...
    categories = {
        ...
        "Binary Log":   {k: v for k, v in merged.items() if "log_bin" in k or "binlog" in k or k in ("sync_binlog",)},
        ...
        "Misc":         {k: v for k, v in merged.items() if k in ("read_only", "super_read_only",
                          "expire_logs_days", "max_binlog_size") or "auto_increment" in k},
    }
...
def _compute_health(r: dict):
    ...
    if r["is_galera"]:
        ...
        try:
            recv_q = float(g.get("local_recv_queue", 0))
            if recv_q > 10:
                issues.append(f"⚠ High receive queue: {recv_q}")
        except Exception:
            pass
    ...
    if r["is_slave"]:
        ...
        if ss.get("last_error"):
            issues.append(f"✗ Error: {ss['last_error']}")
        if ss.get("last_io_error"):
            issues.append(f"✗ IO error: {ss['last_io_error']}")
        if ss.get("last_sql_error"):
            issues.append(f"✗ SQL error: {ss['last_sql_error']}")
    ...
    if issues:
        health = "critical" if any("✗" in i for i in issues) else "warning"
    else:
        health = "healthy"
