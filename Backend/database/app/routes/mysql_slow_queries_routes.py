...
def get_slow_queries(conn_id: int, db: Session = Depends(get_db)):
    ...
    _cached = _get_snap(conn_id, "mysql_slow_queries", db)
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # ── Parse slow log file (still inside with conn:) ─────
            file_queries, file_error = [], None
            full_slow_file = slow_file
            if slow_log == "ON" and slow_file:
                # Step 1: resolve full absolute path (MariaDB often returns just filename)
                ...
                # Step 2: try local read (backend on same host as MySQL)
                if os.path.exists(full_slow_file):
                    ...
                    # Step 3: read via MySQL LOAD_FILE() — works for remote servers
                    try:
                        ...
                        if raw is not None:
                            ...
                            # LOAD_FILE() returned NULL — diagnose why
                            has_file_priv = False
                            try:
                ...
                # Step 4: SSH fallback — cat the file over SSH
                if not file_queries and rec.ssh_user and rec.ssh_password:
                    ...
                    if content_str:
                        file_queries = _parse_slow_log_content(content_str, rec.database_name or "")
                        file_error = None  # clear previous error — SSH succeeded
                    else:
...
# ── Slow-log parser (MySQL + MariaDB compatible) ─────────────────────────────

def _parse_slow_log_content(content: str, default_db: str) -> list:
    """
    Parse MySQL/MariaDB slow query log content from a string.
    Handles both MySQL (#Time: ISO format) and MariaDB (#Time: YYMMDD HH:MM:SS).
    Also works when SET timestamp= line is absent.
    """
    queries = []
    # Split on the Time marker; some blocks before first marker have no queries
    for block in content.split("# Time:"):
        try:
            ...
            # ── database: prefer Schema: field (MariaDB), then USE statement ──
            db_name = default_db
            ...
            # ── SQL extraction ──
            # Try after SET timestamp= (MySQL format, sometimes MariaDB)
            sql_text = ""
            ts_m = re.search(r"SET timestamp=\d+;\n([\s\S]+?)(?:\n#|\Z)", block)
            if ts_m:
                sql_text = ts_m.group(1).strip().rstrip(";")
            else:
                # Fallback: collect non-comment, non-meta lines at end of block
                sql_lines = []
                past_header = False
...
def get_ssh_config(conn_id: int, db: Session = Depends(get_db)):
    """Return stored SSH credentials (password masked) for this connection."""
    rec = db.query(ConnectionMaster).filter(ConnectionMaster.id == conn_id).first()
    if not rec:
        raise HTTPException(404, "Connection not found")
    return {
...
def save_ssh_config(conn_id: int, payload: SSHConfigRequest, db: Session = Depends(get_db)):
    """Save SSH credentials for slow-log remote file reading."""
    rec = db.query(ConnectionMaster).filter(ConnectionMaster.id == conn_id).first()
    if not rec:
        raise HTTPException(404, "Connection not found")

    # Test SSH before saving
    ssh_host = payload.ssh_host or rec.host
    content, err = _read_file_via_ssh(ssh_host, payload.ssh_port or 22, payload.ssh_user, payload.ssh_password, "/etc/hostname")
    if err and not content:
        return {"status": "error", "message": f"SSH test failed: {err}"}

    rec.ssh_host     = payload.ssh_host or None
    rec.ssh_port     = payload.ssh_port or 22
    rec.ssh_user     = payload.ssh_user
    rec.ssh_password = payload.ssh_password
    db.commit()
...
def analyze_slow_query_groq(
    conn_id: int,
    payload: SlowQueryGroqRequest,
    db: Session = Depends(get_db),
):
    """Deep Groq LLM analysis for a single slow query with its EXPLAIN output."""
    rec = db.query(ConnectionMaster).filter(ConnectionMaster.id == conn_id).first()
    if not rec:
    ...
    try:
        ...
        efficiency = (
            round(payload.rows_returned / max(payload.rows_examined, 1) * 100, 1)
            if payload.rows_examined else 100
        )
...
=== PERFORMANCE METRICS ===
Execution Count: {payload.count_calls:,}
Average Execution Time: {payload.avg_exec_sec:.4f}s
Maximum Execution Time: {payload.max_exec_sec:.4f}s
Total Cumulative Time: {payload.total_exec_sec:.4f}s
Rows Examined: {payload.rows_examined:,}
Rows Returned: {payload.rows_returned:,}
Efficiency Ratio: {efficiency}% (rows_returned / rows_examined)
No-Index Scans: {payload.no_index_count}
...
# ── EXPLAIN with analysis endpoint ────────────────────────────────────────────
...
def explain_and_analyze(
    conn_id: int,
    payload: ExplainAnalysisRequest,
    db: Session = Depends(get_db),
):
    """Run EXPLAIN on a query and return structured analysis hints."""
    rec = db.query(ConnectionMaster).filter(ConnectionMaster.id == conn_id).first()
    if not rec:
        return {"status": "error", "error": "Connection not found"}

    pw = quote_plus(rec.password or "")
    db_name = payload.db_name or rec.database_name or ""
    url = f"mysql+pymysql://{rec.username}:{pw}@{rec.host}:{rec.port}/{db_name}"
        ...
        with engine.connect() as conn:
            if db_name:
                try:
                    ...
                    pass
            rows = conn.execute(text(f"EXPLAIN {payload.sql_text}")).fetchall()
...
def export_slow_queries_report(
    ...
    """Export slow queries as a CSV report filtered by time period."""
    rec = db.query(ConnectionMaster).filter(ConnectionMaster.id == conn_id).first()
    if not rec:
        raise HTTPException(404, "Connection not found")
    ...
    try:
        with engine.connect() as conn:
            # Try performance schema first
            where_clause = ""
            if hours:
                where_clause = f"AND LAST_SEEN >= NOW() - INTERVAL {hours} HOUR"
            ...
            # Fallback: read slow query log file (local or via LOAD_FILE for remote)
            if not queries:
                try:
                    var_rows = conn.execute(text(
