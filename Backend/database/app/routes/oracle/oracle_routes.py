...
def list_oracle_connections(db: Session = Depends(get_db)):
    return oracle_connection_service.list_connections(db)


@router.post("/")
def create_oracle_connection(request: OracleConnectionCreate, db: Session = Depends(get_db)):
    return oracle_connection_service.create_connection(request, db)
