"""
PostgreSQL — Backup, Restore & Point-in-Time Recovery
======================================================
Backup types
  logical    → pg_dump (custom .dump) or pg_dumpall (.sql.gz)
  basebackup → pg_basebackup (physical; required for WAL PITR)
  wal        → WAL segment archive from pg_wal/

PITR flow
  1. pg_basebackup creates consistent base image
  2. WAL archive fills the gap to recovery_target_time
  3. recovery.signal + postgresql.auto.conf (PG≥12) or recovery.conf (PG<12)
  4. New PG instance starts on alternate port, recovers, then promotes

All heavy work runs on the PostgreSQL server via SSH/SFTP.

Endpoints (prefix /api/v1/connections/postgresql)
--------------------------------------------------
GET    /{id}/backup/summary
GET    /{id}/backup/schedules          ← before /{id}/backup/{job_id}
POST   /{id}/backup/schedules
GET    /{id}/backup/schedules/{sid}
PUT    /{id}/backup/schedules/{sid}
DELETE /{id}/backup/schedules/{sid}
PATCH  /{id}/backup/schedules/{sid}/toggle
POST   /{id}/backup/schedules/{sid}/run-now
GET    /{id}/backups
POST   /{id}/backup/take
GET    /{id}/backup/{job_id}           ← after all literal routes
...
import json, shutil, tarfile, threading, uuid
...
router = APIRouter(
    ...
    tags=["PostgreSQL Backup & PITR"],
)

# Ensure backup_schedules table at import time
try:
...
class TakeBackupRequest(BaseModel):
    backup_type: str = "logical"           # logical | basebackup | wal
    databases: Optional[List[str]] = None  # None = all (logical only)
    dump_format: str = "custom"            # custom | plain (for pg_dump)
    compress: bool = True
...
class RestoreRequest(BaseModel):
    ...
    create_db: bool = True
    clean: bool = True                     # --clean --if-exists
    no_owner: bool = True
    confirm: bool = False

class PITRRequest(BaseModel):
    base_job_id: int                       # basebackup job to restore from
    target_datetime: str                   # "YYYY-MM-DD HH:MM:SS"
    target_action: str = "pause"           # pause | promote | shutdown
    recovery_port: Optional[int] = None   # port for recovery instance (default primary+1)
    wal_archive_path: Optional[str] = None # local WAL archive dir on server
    confirm: bool = False
...
def _pg_engine(rec: ConnectionMaster):
    ...
    db  = rec.database_name or "postgres"
    return create_engine(
        f"postgresql+psycopg2://{usr}:{pw}@{rec.host}:{rec.port or 5432}/{db}",
        pool_pre_ping=True,
...
def _job_dict(j: BackupJob) -> dict:
    ...
    return {
        ...
        "wal_file":      j.binlog_file,   # reused field: WAL file name at backup time
        "start_lsn":     j.binlog_pos,    # reused field: start LSN as bigint
...
def _do_pg_logical(job_id: int, conn_id: int, custom_path: Optional[str]):
    """pg_dump (custom format) or pg_dumpall via SSH."""
    db = SessionLocal()
    try:
        ...
        # Record WAL position before backup starts
        try:
            eng = _pg_engine(rec)
        ...
        if not ssh:
            job.status = "failed"
            job.error_msg = "SSH unavailable — add SSH credentials in the OS Server for this host"
        ...
        if dbn == "ALL":
            cmd = (
                f"PGPASSWORD='{pw}' pg_dumpall -h 127.0.0.1 -p {port} -U '{user}' "
                f"--clean --if-exists 2>/tmp/actmon_pgdump_err.txt | gzip"
            )
            ext = "sql.gz"
        else:
            # Custom format — already compressed, output directly
            cmd = (
                f"PGPASSWORD='{pw}' pg_dump -h 127.0.0.1 -p {port} -U '{user}' "
                f"-Fc --no-password '{dbn}' 2>/tmp/actmon_pgdump_err.txt"
            )
            ext = "dump"

        out_bytes, err_str, rc = _ssh_run(ssh, cmd, timeout=7200)

        if rc != 0 or len(out_bytes) < 100:
            _, err_raw, _ = _ssh_run(ssh, "cat /tmp/actmon_pgdump_err.txt 2>/dev/null")
            job.status    = "failed"
            job.error_msg = (
                err_raw.decode("utf-8", errors="replace")
                if isinstance(err_raw, bytes) else err_raw
            ) or err_str or "pg_dump failed with no output"
            job.backup_end = datetime.utcnow()
            db.commit()
            ssh.close()
            return
...
def _do_pg_basebackup(job_id: int, conn_id: int, custom_path: Optional[str]):
    """pg_basebackup (tar+gz) via SSH; downloads archive via SFTP."""
    db = SessionLocal()
    try:
        job = db.query(BackupJob).filter(BackupJob.id == job_id).first()
        rec = db.query(ConnectionMaster).filter(ConnectionMaster.id == conn_id).first()
        ...
        ssh = _get_ssh(rec)
        if not ssh:
            job.status = "failed"; job.error_msg = "SSH unavailable"; job.backup_end = datetime.utcnow()
            db.commit()
            return
        ...
        # pg_basebackup: tar format + gzip, stream WAL with checkpoint=fast
        cmd = (
            f"PGPASSWORD='{pw}' pg_basebackup -h 127.0.0.1 -p {port} -U '{user}' "
            f"-D '{tmp}' -Ft -z --wal-method=stream --checkpoint=fast -P "
            f"2>/tmp/actmon_pgbase_err.txt && echo __PGBASE_OK__"
        )
        _, out_str, rc = _ssh_run(ssh, cmd, timeout=7200)
        out_text = out_str if isinstance(out_str, str) else out_str.decode("utf-8", errors="replace")

        if rc != 0 or "__PGBASE_OK__" not in out_text:
            _, err_raw, _ = _ssh_run(ssh, "tail -60 /tmp/actmon_pgbase_err.txt 2>/dev/null")
            err_txt = err_raw.decode("utf-8", errors="replace") if isinstance(err_raw, bytes) else err_raw
            job.status    = "failed"
            job.error_msg = err_txt[:2000] or "pg_basebackup failed"
            job.backup_end = datetime.utcnow()
            db.commit()
            _ssh_run(ssh, f"rm -rf '{tmp}'")
            ssh.close()
            return

        # Pack the tmp dir into a single tar.gz for download
        ts       = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        tmp_arch = f"/tmp/actmon_pgbase_{job.uuid}_{ts}.tar.gz"
        _, _, tar_rc = _ssh_run(ssh, f"tar czf '{tmp_arch}' -C '{tmp}' . 2>&1", timeout=600)

        if tar_rc != 0:
            job.status = "failed"; job.error_msg = "Could not pack basebackup"
            job.backup_end = datetime.utcnow(); db.commit()
            _ssh_run(ssh, f"rm -rf '{tmp}' '{tmp_arch}'")
            ssh.close()
            return
        ...
        sftp = ssh.open_sftp()
        sftp.get(tmp_arch, str(fpath))
        sftp.close()
        _ssh_run(ssh, f"rm -rf '{tmp}' '{tmp_arch}'")

        # Parse backup_label inside the archive for start LSN
        try:
...
def _do_pg_wal_archive(job_id: int, conn_id: int, custom_path: Optional[str]):
    """Archive current WAL segments from pg_wal/ via SFTP."""
    db = SessionLocal()
    try:
        job = db.query(BackupJob).filter(BackupJob.id == job_id).first()
        rec = db.query(ConnectionMaster).filter(ConnectionMaster.id == conn_id).first()
        ...
        ssh = _get_ssh(rec)
        if not ssh:
            job.status = "failed"; job.error_msg = "SSH unavailable"
            job.backup_end = datetime.utcnow(); db.commit()
            return

        # Switch WAL to archive current segment
        try:
            eng = _pg_engine(rec)
        ...
        # Find data directory and pg_wal
        pw   = (rec.password or "").replace("'", "'\\''")
        port = rec.port or 5432
        user = rec.username or "postgres"
        ...
        _, dd_out, _ = _ssh_run(
            ...
            f"-t -c \"SHOW data_directory\" postgres 2>/dev/null | xargs echo",
            timeout=15,
        )
        data_dir = (dd_out.strip() if isinstance(dd_out, str) else dd_out.decode("utf-8", errors="replace").strip())
        wal_dir  = f"{data_dir}/pg_wal" if data_dir else "/var/lib/postgresql/data/pg_wal"

        _, ls_out, _ = _ssh_run(ssh, f"ls '{wal_dir}' 2>/dev/null | grep -E '^[0-9A-F]{{24}}$'")
        wal_files = [
            f.strip()
            for f in (ls_out if isinstance(ls_out, str) else ls_out.decode("utf-8", errors="replace")).splitlines()
            if f.strip()
        ]
        ...
        if not wal_files:
            ...
            job.backup_end = datetime.utcnow(); db.commit(); ssh.close()
            return
        ...
        _, _, tar_rc = _ssh_run(ssh, f"cd '{wal_dir}' && tar czf '{tmp_arch}' {file_list} 2>&1", timeout=600)

        if tar_rc != 0:
            job.status = "failed"; job.error_msg = "Failed to archive WAL segments"
            job.backup_end = datetime.utcnow(); db.commit()
            _ssh_run(ssh, f"rm -f '{tmp_arch}'"); ssh.close()
            return
...
def _do_pg_restore(job_id: int, restore_job_id: int, conn_id: int,
    ...
    """Restore .dump (pg_restore) or .sql.gz (psql) on the remote server."""
    db = SessionLocal()
    try:
        ...
        ssh = _get_ssh(rec)
        if not ssh:
            rm.status = "failed"; rm.error_msg = "SSH unavailable"
            rm.backup_end = datetime.utcnow(); db.commit(); return
        ...
        if create_db:
            _ssh_run(ssh, (
                ...
                f"-U '{user}' postgres -c \"CREATE DATABASE \\\"{t_db}\\\"\" 2>&1"
            ), timeout=30)

        if src_path.suffix == ".gz":
            # pg_dumpall output — plain SQL
            cmd = (
                f"PGPASSWORD='{pw}' zcat '{tmp_remote}' | "
            ...
            cmd = (
                f"PGPASSWORD='{pw}' pg_restore -h 127.0.0.1 -p {port} -U '{user}' "
                f"-d '{t_db}' -Fc{flags} --single-transaction "
                f"'{tmp_remote}' 2>&1"
            )

        _, out, rc = _ssh_run(ssh, cmd, timeout=7200)
        out_text = out.decode("utf-8", errors="replace") if isinstance(out, bytes) else str(out)
        _ssh_run(ssh, f"rm -f '{tmp_remote}'")
        ssh.close()

        if rc != 0 and "ERROR" in out_text.upper():
...
def _do_pg_pitr(pitr_job_id: int, base_job_id: int, conn_id: int,
    ...
    """WAL-based PITR: extract base backup → config recovery → start on alt port."""
    db = SessionLocal()
    try:
        ...
        ssh = _get_ssh(rec)
        if not ssh:
            pj.status = "failed"; pj.error_msg = "SSH unavailable"
            pj.backup_end = datetime.utcnow(); db.commit(); return
        ...
        # ── 1: Upload base backup ─────────────────────────────────────────────
        pj.notes = "Step 1/4: Uploading base backup…"; db.commit()
        sftp = ssh.open_sftp()
        sftp.put(str(base.file_path), arch)
        sftp.close()

        # ── 2: Extract to recovery data dir ──────────────────────────────────
        pj.notes = "Step 2/4: Extracting base backup…"; db.commit()
        _ssh_run(ssh, f"rm -rf '{rdir}' && mkdir -p '{rdir}'")
        _, _, tar_rc = _ssh_run(ssh, f"tar xzf '{arch}' -C '{rdir}' 2>&1", timeout=600)
        _ssh_run(ssh, f"rm -f '{arch}'")

        if tar_rc != 0:
            pj.status = "failed"; pj.error_msg = "Failed to extract base backup"
            pj.backup_end = datetime.utcnow(); db.commit()
            _ssh_run(ssh, f"rm -rf '{rdir}'"); ssh.close(); return

        # ── 3: Detect PG version + write recovery config ─────────────────────
        ...
        if pg_ver_num >= 120000:
            # PG 12+: postgresql.auto.conf + recovery.signal
            auto_conf = (
            ...
            # Write config safely via here-doc
            _ssh_run(ssh, f"cat >> '{rdir}/postgresql.auto.conf' << 'ACTMON_CONF'\n{auto_conf}\nACTMON_CONF")
            _ssh_run(ssh, f"touch '{rdir}/recovery.signal'")
            # Remove standby.signal if present
            _ssh_run(ssh, f"rm -f '{rdir}/standby.signal'")
        else:
            # PG < 12: recovery.conf
            rec_conf = (
                f"restore_command = '{restore_cmd_str}'\n"
            ...
            _ssh_run(ssh, f"cat > '{rdir}/recovery.conf' << 'ACTMON_CONF'\n{rec_conf}\nACTMON_CONF")

        # Fix permissions
        _ssh_run(ssh, f"chown -R postgres:postgres '{rdir}' 2>/dev/null; chmod 700 '{rdir}'")

        # ── 4: Start recovery instance ────────────────────────────────────────
        pj.notes = f"Step 4/4: Starting recovery instance on port {rport}…"; db.commit()

        _, pg_ctl_raw, _ = _ssh_run(ssh, "which pg_ctl 2>/dev/null || find /usr/lib/postgresql -name pg_ctl 2>/dev/null | head -1")
        pg_ctl = (pg_ctl_raw.strip() if isinstance(pg_ctl_raw, str) else pg_ctl_raw.decode("utf-8", errors="replace").strip()) or "pg_ctl"
        log    = f"/tmp/actmon_pgpitr_{pj.uuid}.log"

        start_cmd = f"sudo -u postgres {pg_ctl} -D '{rdir}' -l '{log}' start 2>&1"
        _, start_out, start_rc = _ssh_run(ssh, start_cmd, timeout=120)
        start_txt = start_out if isinstance(start_out, str) else start_out.decode("utf-8", errors="replace")

        if start_rc != 0 and "already running" not in start_txt.lower():
            pj.status    = "failed"
            pj.error_msg = f"Could not start recovery instance: {start_txt[:800]}"
            pj.backup_end = datetime.utcnow(); db.commit(); ssh.close(); return
        ...
        pj.backup_end = datetime.utcnow(); db.commit(); ssh.close()
...
# ═══════════════════════════════════════════════════════════════════════════════
#  API Endpoints
#  Order matters: literal routes MUST appear before /{conn_id}/backup/{job_id}
# ═══════════════════════════════════════════════════════════════════════════════

# ── Summary ───────────────────────────────────────────────────────────────────
@router.get("/{conn_id}/backup/summary")
def pg_backup_summary(conn_id: int, db: Session = Depends(_get_db)):
    ...
    try:
        ...
        wal_info = _rows(eng, """
            SELECT
                ...
                pg_wal_lsn_diff(pg_current_wal_lsn(),
                                '0/0'::pg_lsn)::bigint        AS lsn_bytes,
        ...
        """)
        wal_info = wal_info[0] if wal_info else {}

        archiver = _rows(eng, """
            SELECT archived_count, last_archived_wal, last_archived_time::text,
                   failed_count, last_failed_wal, last_failed_time::text,
                   stats_reset::text
            FROM pg_stat_archiver
        """)
        archiver = archiver[0] if archiver else {}
    ...
    return {
        ...
        "wal":          {str(k): str(v) if v is not None else None for k, v in (wal_info.items() if wal_info else {}.items())},
        "archiver":     {str(k): str(v) if v is not None else None for k, v in (archiver.items() if archiver else {}.items())},
...
# ── Schedule CRUD — BEFORE /{conn_id}/backup/{job_id} ─────────────────────────
@router.get("/{conn_id}/backup/schedules")
def list_pg_schedules(conn_id: int, db: Session = Depends(_get_db)):
    try:
        rows = (db.query(BackupSchedule)
                .filter(BackupSchedule.conn_id == conn_id)
                .order_by(BackupSchedule.id.desc()).all())
...
def create_pg_schedule(conn_id: int, req: ScheduleCreate, db: Session = Depends(_get_db)):
...
def get_pg_schedule(conn_id: int, sid: int, db: Session = Depends(_get_db)):
...
def update_pg_schedule(conn_id: int, sid: int, req: ScheduleUpdate, db: Session = Depends(_get_db)):
    s = db.query(BackupSchedule).filter(BackupSchedule.id == sid, BackupSchedule.conn_id == conn_id).first()
    if not s:
        raise HTTPException(404, "Schedule not found")
...
def delete_pg_schedule(conn_id: int, sid: int, db: Session = Depends(_get_db)):
    s = db.query(BackupSchedule).filter(BackupSchedule.id == sid, BackupSchedule.conn_id == conn_id).first()
    if not s:
        raise HTTPException(404, "Schedule not found")
...
def toggle_pg_schedule(conn_id: int, sid: int, req: ScheduleToggle, db: Session = Depends(_get_db)):
    s = db.query(BackupSchedule).filter(BackupSchedule.id == sid, BackupSchedule.conn_id == conn_id).first()
    if not s:
        raise HTTPException(404, "Schedule not found")
...
def run_pg_schedule_now(conn_id: int, sid: int, db: Session = Depends(_get_db)):
    s = db.query(BackupSchedule).filter(BackupSchedule.id == sid, BackupSchedule.conn_id == conn_id).first()
    if not s:
        raise HTTPException(404, "Schedule not found")
...
def list_pg_backups(conn_id: int, db: Session = Depends(_get_db)):
    _get_conn(conn_id, db)
    jobs = (db.query(BackupJob)
            .filter(BackupJob.connection_id == conn_id)
...
def take_pg_backup(conn_id: int, req: TakeBackupRequest,
                   background_tasks: BackgroundTasks, db: Session = Depends(_get_db)):
    rec = _get_conn(conn_id, db)
...
# ── Single job + delete — AFTER all literal routes ────────────────────────────
@router.get("/{conn_id}/backup/{job_id}")
def get_pg_backup(conn_id: int, job_id: int, db: Session = Depends(_get_db)):
...
def delete_pg_backup(conn_id: int, job_id: int, db: Session = Depends(_get_db)):
    j = db.query(BackupJob).filter(BackupJob.id == job_id, BackupJob.connection_id == conn_id).first()
    if not j:
        raise HTTPException(404, "Backup job not found")
    if j.status == "running":
...
def pg_restore(conn_id: int, req: RestoreRequest,
               background_tasks: BackgroundTasks, db: Session = Depends(_get_db)):
    if not req.confirm:
        raise HTTPException(400, "Set confirm=true to proceed")
    rec     = _get_conn(conn_id, db)
    src_job = db.query(BackupJob).filter(
        BackupJob.id == req.job_id, BackupJob.connection_id == conn_id).first()
    if not src_job:
...
def pg_pitr(conn_id: int, req: PITRRequest,
            background_tasks: BackgroundTasks, db: Session = Depends(_get_db)):
    if not req.confirm:
    ...
    base_job = db.query(BackupJob).filter(
        BackupJob.id == req.base_job_id, BackupJob.connection_id == conn_id).first()
    if not base_job:
...
def pg_pitr_preview(conn_id: int, db: Session = Depends(_get_db)):
    _get_conn(conn_id, db)
    rec = db.query(ConnectionMaster).filter(ConnectionMaster.id == conn_id).first()
...
def pg_wal_status(conn_id: int, db: Session = Depends(_get_db)):
    rec = _get_conn(conn_id, db)
    try:
        eng = _pg_engine(rec)
        wal = (_rows(eng, """
...
def pg_wal_segments(conn_id: int, db: Session = Depends(_get_db)):
    rec = _get_conn(conn_id, db)
    ssh = _get_ssh(rec)
    if not ssh:
        return {"status": "error", "error": "SSH unavailable", "segments": []}
    ...
    _, dd_out, _ = _ssh_run(
        ssh,
        f"PGPASSWORD='{pw}' psql -h 127.0.0.1 -p {port} -U '{user}' postgres "
        f"-t -c \"SHOW data_directory\" 2>/dev/null | xargs echo",
        timeout=15,
    )
    data_dir = (dd_out.strip() if isinstance(dd_out, str) else dd_out.decode("utf-8", errors="replace").strip())
    wal_dir  = f"{data_dir}/pg_wal" if data_dir else "/var/lib/postgresql/data/pg_wal"
    ...
    _, ls_out, _ = _ssh_run(
        ssh,
        f"ls -lh '{wal_dir}' 2>/dev/null | grep -E '[0-9A-F]{{24}}' | "
        f"awk '{{print $5\" \"$9}}'",
    )
    ssh.close()

    segments = []
    for line in (ls_out if isinstance(ls_out, str) else ls_out.decode("utf-8", errors="replace")).splitlines():
