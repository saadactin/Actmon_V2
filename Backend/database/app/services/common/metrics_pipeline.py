...
def record(agent_name, metrics, kind=None, tech=None, conn_id=None, _connection=None):
    ...
    try:
        ...
        # …and hand it to the LOSSLESS queue: it stays in Redis until the background
        # flusher confirms the ClickHouse insert — a CH outage loses nothing.
        if prev:
            if not redis_store.queue_pending(agent_name, prev):
                metrics_history.flush_sample(agent_name, prev)   # direct best-effort fallback
...
def status():
    ...
    return {"enabled": _ENABLED,
            ...
            "clickhouse": ch_ok,
            "pending_ch": redis_store.pending_depth(),   # queued samples awaiting CH
            "tables": metrics_history.list_tables() if ch_ok else [],
            "ring_size": redis_store.RING_SIZE,
            "ch_ttl_days": metrics_history.TTL_DAYS}


# ── Background flusher: drains the lossless Redis→ClickHouse queue ─────────────
...
def _flusher_loop():
    ...
    while True:
        try:
            items = redis_store.drain_pending(batch=500)
            if not items:
                _t.sleep(5)
                continue
            if not metrics_history.flush_batch(items):
                redis_store.requeue_pending(items)     # CH down → nothing lost
                _t.sleep(15)
        except Exception as e:  # noqa: BLE001
            logger.debug("[metrics_pipeline] flusher: %s", e)
            _t.sleep(10)
...
def start_flusher():
    ...
    t = _threading.Thread(target=_flusher_loop, name="metrics-ch-flusher", daemon=True)
    t.start()
    logger.info("[metrics_pipeline] Redis→ClickHouse flusher started (lossless queue)")
...
def record_stable(agent_name, facts, kind="database", tech=None, conn_id=0, db=None):
    ...
    try:
        ...
        # WARNING (not debug): silent failure here made "stable facts empty" a
        # blind spot — surface the reason (e.g., missing table) in the journal.
        logger.warning("[metrics_pipeline] record_stable failed for '%s': %s", agent_name, e)
        try:
            db.rollback()
        except Exception:  # noqa: BLE001
            pass
...
def register_hooks():
    try:
        ...
        from app.models.agent_model import AgentMetric, AgentTopSQL, AgentWaitEvent
        ...
        # ClickHouse is the historical store.
        @event.listens_for(AgentTopSQL, "after_insert")
        def _on_topsql_insert(mapper, connection, target):  # noqa: ANN001
            try:
                redis_store.queue_pending_typed("top_sql", target.agent_name, {
                    "sql_id": target.sql_id, "sql_text": target.sql_text,
                    "executions": target.executions, "avg_elapsed_ms": target.avg_elapsed_ms,
                    "cpu_time_ms": target.cpu_time_ms, "buffer_gets": target.buffer_gets,
                    "total_ms": target.total_ms, "max_ms": target.max_ms})
            except Exception:  # noqa: BLE001
                pass

        @event.listens_for(AgentWaitEvent, "after_insert")
        def _on_wait_insert(mapper, connection, target):  # noqa: ANN001
            try:
                redis_store.queue_pending_typed("wait_event", target.agent_name, {
                    "event_name": target.event_name, "wait_class": target.wait_class,
                    "time_waited_ms": target.time_waited_ms, "avg_ms": target.avg_ms,
                    "count": target.count})
            except Exception:  # noqa: BLE001
                pass

        logger.info("[metrics_pipeline] hooks registered: metrics + top_sql + wait_events "
                    "(Redis hot tier + lossless ClickHouse queue)")
        start_flusher()
    except Exception as e:  # noqa: BLE001
