...
endpoints can scope reads/writes to one company.
  ...
  ctx = {"org_id": int|None, "role_id": int|None, "user_id": int|None, "is_super": bool}
...
def tenant_ctx(credentials: Optional[HTTPAuthorizationCredentials] = Depends(_bearer)) -> dict:
    if not credentials:
        return {"org_id": None, "role_id": None, "user_id": None, "is_super": True}
    try:
    ...
    return {"org_id": org_id, "role_id": role_id, "user_id": claims.get("user_id"), "is_super": is_super}
