...
def flush_rac_nodes(agent_name, conn_id, response, db):
    ...
    for n in nodes:
        if str(n.get("instance_status") or "").upper() != "OPEN":
            _notify(db, agent_name,
                    f"RAC node {n.get('instance_number')} ({n.get('host_name') or 'unknown host'}) "
                    f"is {n.get('instance_status') or 'unreachable'}, not OPEN",
                    "critical")
