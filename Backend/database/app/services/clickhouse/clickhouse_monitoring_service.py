...
def _agent_query(conn, sql):
    """Agent-hosted ClickHouse (localhost on the DB box) → run the SQL through that
    host's agent. Returns (rows, err) — or (None, None) when NOT agent-linked, so
    the caller falls through to the direct HTTP/native paths unchanged."""
    try:
        from app.database.connection import SessionLocal
        from app.services.common import db_proxy_service
        s = SessionLocal()
        try:
            host = db_proxy_service.agent_host_for_conn(getattr(conn, "id", None), s)
        finally:
            s.close()
        if not host or not getattr(host, "token", None):
            return None, None
        rows, cols = db_proxy_service._agent_query_meta(host.token, conn, sql, timeout=25)
        if rows is None:
            return [], "The agent on the DB host did not answer in time. Check it is online, then refresh."
...
def _query(conn, sql):
    # Agent-linked connections never work direct (backend can't reach the DB host's
    # localhost) — route via the agent and STOP there so errors stay truthful.
    agent_rows, agent_err = _agent_query(conn, sql)
    if agent_rows is not None or agent_err:
        return agent_rows or [], "agent", agent_err
