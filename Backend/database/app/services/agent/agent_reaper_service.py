...
def reap_once():
    ...
    try:
        ...
        # after max(OFFLINE_AFTER, 4 × its own collection interval) of silence — so a
        # slow/busy host that reports every 60s isn't flapped by a fixed 3-min rule,
        # and status changes only after genuinely missed collection cycles.
        _stale_pred = ("status <> 'offline' AND last_heartbeat IS NOT NULL "
                       "AND last_heartbeat < now() - make_interval(secs => "
                       "GREATEST(:s, COALESCE(collection_interval_sec, 60) * 4))")
        going_offline = [row[0] for row in db.execute(text(
            f"SELECT agent_name FROM agents WHERE {_stale_pred}"), {"s": OFFLINE_AFTER}).fetchall()]
        r = db.execute(text(
            f"UPDATE agents SET status='offline' WHERE {_stale_pred}"), {"s": OFFLINE_AFTER})
