...
def _engine(conn: ConnectionMaster):
    # Route through the host agent when the DB is agent-hosted (localhost on that host);
    # otherwise return the real engine unchanged.
...
def get_dashboard(conn_id: int, db: Session, live: bool = False) -> dict:
    ...
    try:
        ...
        with eng.connect() as conn:
            ...
            # A live MySQL ALWAYS returns status + variables. Both empty = the query
            # failed (agent timeout / DB unreachable). Fail loudly so the empty result
            # is NOT cached over good data (see _try_snapshot).
            if not status_dict and not vars_dict:
                raise RuntimeError("MySQL returned no status/variables — connection failed; not caching empty result")
            ...
            # Self-cache this good build so the dashboard stays populated for the TTL
            # even if the collector's next build fails (agent busy).
            try:
                from app.utils.agent_cache import store_snapshot_for_conn
                store_snapshot_for_conn(conn_id, "mysql_dashboard", _res, db)
            except Exception:
                pass
            return _res
