...
# ─────────────────────────────────────────────────────────────
# Oracle collector (stub — uses push API for now)
# ─────────────────────────────────────────────────────────────
    ...
    try:
        ...
        if not dsn and conn_rec.host:
            dsn = cx_Oracle.makedsn(
                conn_rec.host,
                conn_rec.port or 1521,
                service_name=conn_rec.service_name or conn_rec.sid,
            )
        oracle_conn = cx_Oracle.connect(conn_rec.username, conn_rec.password, dsn)
        ...
        cursor.execute("""
            SELECT i.version, i.instance_name, i.host_name,
                   ...
                   d.log_mode, d.open_mode, d.database_status,
        ...
        if row:
            ...
            # Remove existing snapshot older than 1h, keep latest
        ...
        logger.warning(f"[agent_collector] cx_Oracle not installed; Oracle agent '{agent_name}' uses push API.")
        return False
    except Exception as exc:
...
# ─────────────────────────────────────────────────────────────
# Snapshot collectors
# Each DB type has a function that calls the existing route
# handler functions (bypassing their cache check) and stores
# the results as AgentSnapshot records.  Routes then return
# those cached records without ever touching the monitored DB.
# ─────────────────────────────────────────────────────────────

def _try_snapshot(agent_name: str, conn_id: int, snap_type: str, fn, db):
    """Call fn(), persist result. The collector bypass flag is active so route
    handlers skip their own cache check and always query the monitored DB."""
...
def _collect_mysql_snapshots(agent_name: str, conn_id: int, db):
    from app.routes.mysql_routes import (
        get_mysql_dashboard, get_table_stats, get_innodb_metrics,
        get_user_stats, get_performance_detail, get_backup_info,
    )
    from app.routes.mysql_slow_queries_routes import get_slow_queries
    from app.routes.mysql_replication_routes import replication_status, replication_variables
    from app.routes.mysql_index_analysis_routes import index_analysis
...
def _collect_postgres_snapshots(agent_name: str, conn_id: int, db):
    from app.routes.postgres_monitoring_routes import (
        monitoring_dashboard, pg_slow_queries, pg_index_analysis,
        replication_detail, queries_detail, tables_detail,
        pg_config_detail, users_detail, storage_detail,
    )

    for snap_type, fn in [
        ("pg_monitoring_dashboard", lambda: monitoring_dashboard(conn_id, db)),
        ("pg_slow_queries",         lambda: pg_slow_queries(conn_id, db)),
        ("pg_index_analysis",       lambda: pg_index_analysis(conn_id, db)),
        ("pg_replication_detail",   lambda: replication_detail(conn_id, db)),
        ("pg_queries_detail",       lambda: queries_detail(conn_id, db)),
        ("pg_tables_detail",        lambda: tables_detail(conn_id, db)),
        ("pg_config_detail",        lambda: pg_config_detail(conn_id, db)),
        ("pg_users_detail",         lambda: users_detail(conn_id, db)),
        ("pg_storage_detail",       lambda: storage_detail(conn_id, db)),
    ]:
        _try_snapshot(agent_name, conn_id, snap_type, fn, db)


def _collect_oracle_snapshots(agent_name: str, conn_id: int, db):
    from app.routes.oracle_monitoring_routes import (
...
def _collect_mssql_snapshots(agent_name: str, conn_id: int, db):
    from app.routes.mssql_monitoring_routes import (
        get_monitoring_dashboard, get_mssql_slow_queries, get_mssql_index_analysis,
    )
...
def _collect_mongo_snapshots(agent_name: str, conn_id: int, db):
    from app.routes.mongo_monitoring_routes import (
        mongo_dashboard, mongo_ops, mongo_collections,
        mongo_indexes, mongo_replication,
    )

    for snap_type, fn in [
        ("mongo_dashboard",   lambda: mongo_dashboard(conn_id, db)),
        ("mongo_ops",         lambda: mongo_ops(conn_id, db)),
        ("mongo_collections", lambda: mongo_collections(conn_id, db)),
        ("mongo_indexes",     lambda: mongo_indexes(conn_id, db)),
        ("mongo_replication", lambda: mongo_replication(conn_id, db)),
    ]:
        _try_snapshot(agent_name, conn_id, snap_type, fn, db)


def _collect_clickhouse_snapshots(agent_name: str, conn_id: int, db):
    from app.routes.clickhouse_monitoring_routes import (
        get_ch_dashboard, get_ch_queries,
    )
        ...
        for agent in agents:
            try:
                if not agent.db_connection_id:
                    # Push-only agent — no server-side collection
                    continue
                ...
                # Populate AgentSnapshot cache — bypass the cache check so
                # route handlers query the monitored DB fresh every cycle.
                if success:
