...
def _do_pg_logical(job_id: int, conn_id: int, custom_path: Optional[str]):
    ...
    try:
        ...
        if rc != 0 or len(out_bytes) < 100:
            ...
            job.status    = "failed"
            job.error_msg = (
                err_raw.decode("utf-8", errors="replace") if isinstance(err_raw, bytes) else err_raw
            ) or err_str or "pg_dump failed with no output"
            job.backup_end = datetime.utcnow(); db.commit(); ssh.close(); return
...
def _do_pg_basebackup(job_id: int, conn_id: int, custom_path: Optional[str]):
    ...
    try:
        ...
        if rc != 0 or "__PGBASE_OK__" not in out_text:
            ...
            job.status = "failed"; job.error_msg = err_txt[:2000] or "pg_basebackup failed"
            job.backup_end = datetime.utcnow(); db.commit()
            _ssh_run(ssh, f"rm -rf '{tmp}'"); ssh.close(); return
        ...
        if tar_rc != 0:
            ...
            job.backup_end = datetime.utcnow(); db.commit()
            _ssh_run(ssh, f"rm -rf '{tmp}' '{tmp_arch}'"); ssh.close(); return
        ...
        sftp = ssh.open_sftp()
        sftp.get(tmp_arch, str(fpath)); sftp.close()
...
def _do_pg_wal_archive(job_id: int, conn_id: int, custom_path: Optional[str]):
    ...
    try:
        ...
        if tar_rc != 0:
            ...
            job.backup_end = datetime.utcnow(); db.commit()
            _ssh_run(ssh, f"rm -f '{tmp_arch}'"); ssh.close(); return
...
def _do_pg_restore(job_id: int, restore_job_id: int, conn_id: int,
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        sftp.put(str(src_path), tmp_remote); sftp.close()
        ...
        _ssh_run(ssh, f"rm -f '{tmp_remote}'"); ssh.close()
...
def _do_pg_pitr(pitr_job_id: int, base_job_id: int, conn_id: int,
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        sftp.put(str(base.file_path), arch); sftp.close()
