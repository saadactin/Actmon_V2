...
def host_metrics(tech: str, conn_id: int, db: Session) -> dict:
    ...
    # come before the local-host shortcut: an agent-linked connection's 'localhost'
    # means the AGENT's machine, not the ActMon server — psutil here would silently
...
def history(tech: str, conn_id: int, db: Session, hours: int = 6) -> dict:
    ...
    res = pg.history(conn_id, db, hours=hours)
    if res.get("samples"):
        return res
    # Fallback for every technology: the resource-sample collector only covers some
    # connections, but the metrics pipeline stores host utilisation for EVERY agent
    # in ClickHouse — serve the host agent's series so History graphs always work.
    try:
        row = db.execute(text(
            "SELECT s.server_name FROM database_instances di "
            "JOIN os_servers s ON s.id = di.server_id "
            "WHERE di.connection_id = :c LIMIT 1"), {"c": conn_id}).first()
        host_agent = row[0] if row else None
        if not host_agent:
            return res
        ...
        if cli is None:
            return res
        r = cli.query(
            "SELECT ts, host_cpu, host_memory FROM actmon.metrics_infra "
            "WHERE agent = %(a)s AND ts > now() - INTERVAL %(h)s HOUR "
            "ORDER BY ts", parameters={"a": host_agent, "h": int(hours)})
        samples = [{
            "id": None,                       # CH samples carry no RCA evidence
            "captured_at": str(x[0]),
            "cpu_pct": float(x[1] or 0),
            "ram_pct": float(x[2] or 0),
            "disk_pct": None,
            "is_event": False,
        } for x in r.result_rows]
        return {"samples": samples, "source": "clickhouse", "host_agent": host_agent}
