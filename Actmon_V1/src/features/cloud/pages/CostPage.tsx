...
import { CostFilterTree, EMPTY_TREE_SELECTION, TreeFilterSelection, matchesTreeSelection } from '../components/CostFilterTree';
import { CostReportPanel } from '../components/CostReportPanel';
import { DiagnosticModal, DiagnosticItem } from '../components/DiagnosticModal';
import { DollarSign, TrendingUp, Sparkles, AlertTriangle, ShieldCheck, ChevronRight, ChevronDown, Loader2, Info, PowerOff, Filter, Download, HelpCircle } from 'lucide-react';
...
export const CostPage = () => {
  ...
  const [treeSelection, setTreeSelection] = useState<TreeFilterSelection>(EMPTY_TREE_SELECTION);
  const [expandedStoppedRow, setExpandedStoppedRow] = useState<string | null>(null);
  ...
  const stoppedInstances: any[] = analytics?.stopped_instances || [];
  ...
  const costDiagnosticItems: DiagnosticItem[] = (analytics?.cost_diagnostics || []).map((d: any) => ({
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={trends} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                    ...
                    <YAxis
                      ...
                      tickLine={false}
                      width={64}
                      tickFormatter={(v: number) => `${trendCurrencySymbol}${formatCompactAxisNumber(Number(v))}`}
                    />
                    <Tooltip
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {filteredStopped.length === 0 ? (
                          ...
                          filteredStopped.map(inst => {
                            ...
                            return (
                              <React.Fragment key={inst.resource_id}>
                                <tr
                                  ...
                                  <tr className="bg-gray-50/60">
                                    <td colSpan={7} className="px-4 py-2.5">
                                      <div className="pl-6 space-y-1">
                                        {inst.attached_storage.map((sr: any, idx: number) => (
