 ...
 */
const JobWatcher: React.FC<{ accountId: string; jobId: string }> = ({ accountId, jobId }) => {
  ...
  useEffect(() => {
    if (job?.status === 'COMPLETED') {
      ...
      addToast(`Discovery failed: ${job.error_message || 'Unknown error'}`, 'error');
      setActiveDiscoveryJob(accountId, null);
    }
