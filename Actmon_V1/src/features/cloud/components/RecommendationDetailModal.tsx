...
const FALLBACK_GUIDE: RuleGuide = {
...
const symbolFor = (code: string | null | undefined): string =>
  code ? (({ USD: '$', INR: '₹', EUR: '€', GBP: '£' } as Record<string, string>)[code] ?? `${code} `) : '';
...
export const RecommendationDetailModal: React.FC<RecommendationDetailModalProps> = ({ opt, onClose }) => {
  ...
  const breakdown: any[] = opt.savings_breakdown || [];
  const assumptions: string[] = opt.savings_assumptions || [];
