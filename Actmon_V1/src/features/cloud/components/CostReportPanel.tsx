...
export const CostReportPanel: React.FC<Props> = ({ accountId, open: openProp, onOpenChange }) => {
  ...
  const [groupBy, setGroupBy] = useState<'service' | 'resource'>('resource');
    ...
    try {
      const pdfColumns = byResource
        ? [
            ...
            { key: 'size_gb', label: 'GB', width: 18, align: 'right' as const },
            { key: 'attached_to_name', label: 'Attached To', width: 48 },
            { key: 'cost', label: 'Cost', width: 27, align: 'right' as const },
          ]
        : [
