...
export default function AddConnectionPage() {
  ...
  // Pre-fill from query params when arriving from "Setup Connection" on a server page
  ...
  // Pre-fill host/port/name from URL query params
  useEffect(() => {
  ...
  const handleTestConnection = async (
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: error?.message || error?.response?.data?.detail || 'Connection failed',
      });
  ...
  const handleSaveConnection = async (
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: error?.message || error?.response?.data?.detail || 'Failed to create connection',
      });
        ...
        <div className="bg-white rounded-2xl border border-slate-200">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <InputField
                label="Database Name (optional — leave blank to test connectivity only)"
              >

                <input
                  {...register('databaseName')}
                  placeholder="Leave blank, or enter e.g. mydb"
                  className={inputClass}
                  autoComplete="off"
                />
