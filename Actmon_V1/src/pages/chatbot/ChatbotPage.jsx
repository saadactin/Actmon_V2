            ...
            <div className="px-5 pb-4 pt-1 border-t border-slate-50 bg-slate-50/60 space-y-2">
              {hasNav && (
                ...
                >
                  ...
                  Open {msg.navResult.title}
                  <span className="ml-auto text-emerald-400 text-xs font-normal font-mono">{msg.navResult.path}</span>
                </button>
              )}
              {msg.actions?.map((a,i)=>{
...
/* ── Conversation history (persisted to localStorage, ChatGPT-style) ── */
const CONVO_KEY = 'actmon_ai_convos';
const loadConvos = () => { try { const a = JSON.parse(localStorage.getItem(CONVO_KEY) || '[]'); return Array.isArray(a) ? a : []; } catch { return []; } };
const saveConvos = (a) => { try { localStorage.setItem(CONVO_KEY, JSON.stringify(a.slice(0, 50))); } catch (_) {} };
const titleOf = (msgs) => {
...
export default function ChatbotPage() {
  ...
  const [toolOpen, setToolOpen]       = useState(null);   // bottom accordion: 'library' | 'export' | null
  ...
  // pick the initial conversation once (incoming from widget → new; else most-recent; else fresh)
  useEffect(() => {
    if ((location.state?.messages || []).length > 0) { setActiveId('c' + Date.now()); return; }
    const existing = loadConvos().filter((c) => c.messages?.length > 0).sort((a, b) => (b.ts || 0) - (a.ts || 0));
    if (existing.length > 0) { const c = existing[0]; setActiveId(c.id); setMessages(c.messages); setSuggestions(c.suggestions || []); }
    else setActiveId('c' + Date.now());
  }, []);

  // persist the active conversation whenever its messages change (not mid-stream)
  useEffect(() => {
    if (!activeId) return;
    if (messages.length && messages[messages.length - 1]?.streaming) return;  // wait until the reply finishes
    setConversations((prev) => {
      const idx = prev.findIndex((c) => c.id === activeId);
      let next;
      if (!messages.length) {
        if (idx < 0) return prev;
        next = prev.filter((c) => c.id !== activeId);          // drop emptied chat
      } else {
        const entry = { id: activeId, title: titleOf(messages), messages, suggestions, ts: Date.now() };
        if (idx >= 0) { next = [...prev]; next[idx] = entry; } else { next = [entry, ...prev]; }
      }
      saveConvos(next);
      return next;
    });
  }, [messages, suggestions, activeId]);

  const newChat = () => { setActiveId('c' + Date.now()); setMessages([]); setSuggestions([]); setInput(''); setTimeout(() => inputRef.current?.focus(), 60); };
  const openChat = (c) => { if (c.id === activeId) return; setActiveId(c.id); setMessages(c.messages || []); setSuggestions(c.suggestions || []); };
  ...
  const send = useCallback(async (text = input.trim()) => {
    ...
    // Navigation detection (any page in the app, natural language)
    const navResult = resolveNavigation(text, connections);
    if (navResult) {
      addMsg({ role:'user', content:text });
      addMsg({ role:'assistant', content:`Taking you to **${navResult.title}**.`, streaming:false, navResult, timestamp:new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}) });
      setTimeout(()=>navigate(navResult.path), 500);
      return;
    }
  ...
  // Filter conversation history by search
  const q = search.trim().toLowerCase();
  const shownConvos = conversations
    .filter((c) => c.messages && c.messages.length > 0)
    .filter((c) => !q || (c.title || '').toLowerCase().includes(q) || c.messages.some((m) => (m.content || '').toLowerCase().includes(q)))
    .sort((a, b) => (b.ts || 0) - (a.ts || 0));

  return (
    <div className="-mx-6 md:-mx-8 -mb-6 md:-mb-8 flex flex-col h-[calc(100vh-48px)] bg-brand-bg">
      <PageHeader
        icon={Brain}
        title="ActMon AI"
        subtitle="Your DBA assistant — analyze databases, write SQL, diagnose issues, and navigate instantly"
        crumbs={[{ label: 'ChatBot' }]}
        accent="blue"
      />
      <div className="flex-1 min-h-0 flex gap-0 bg-slate-50 overflow-hidden border-t border-slate-200">
      ...
      <div className="w-72 flex-shrink-0 border-r border-slate-200 bg-white flex flex-col overflow-hidden">
        ...
        <div className="flex-shrink-0 px-3 pt-3 pb-2.5 border-b border-slate-100">
          ...
          <button onClick={newChat}
            className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl text-sm font-bold text-white shadow-sm hover:shadow-md hover:brightness-105 active:scale-[0.99] transition-all"
            style={{background:'linear-gradient(135deg,#4f46e5,#7c3aed)'}}>
          ...
          <div className="relative mt-2">
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)}
              ...
              className="w-full text-xs border border-slate-200 rounded-xl pl-9 pr-3 py-2 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 bg-slate-50 transition-all"/>
          </div>
        </div>

        {/* Conversation history */}
        <div className="flex-1 overflow-y-auto px-2 py-2">
          <p className="px-2 pb-1 text-[10px] font-black text-slate-400 uppercase tracking-widest">Chats</p>
          {shownConvos.length === 0 ? (
            <div className="px-3 py-10 text-center">
              <MessageSquare size={22} className="mx-auto text-slate-300 mb-2"/>
              <p className="text-xs text-slate-400 leading-relaxed">{search ? 'No matching chats.' : 'No conversations yet.'}<br/>{!search && 'Start a new chat above.'}</p>
            </div>
          ) : shownConvos.map(c=>(
            <div key={c.id} onClick={()=>openChat(c)}
              className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl mb-0.5 cursor-pointer transition-colors ${c.id===activeId ? 'bg-indigo-50 ring-1 ring-indigo-100' : 'hover:bg-slate-100'}`}>
              <MessageSquare size={14} className={`flex-shrink-0 ${c.id===activeId ? 'text-indigo-500' : 'text-slate-400'}`}/>
              <span className={`flex-1 truncate text-[13px] font-medium ${c.id===activeId ? 'text-indigo-800' : 'text-slate-700'}`}>{c.title || 'New chat'}</span>
              <button onClick={(e)=>deleteChat(c.id,e)} title="Delete chat"
                className="opacity-0 group-hover:opacity-100 p-1 rounded-md text-slate-400 hover:bg-red-100 hover:text-red-600 transition-all flex-shrink-0">
                <Trash2 size={13}/>
              </button>
            </div>
          ))}
        ...
        <div className="flex-shrink-0 border-t border-slate-100">
          {/* DB context */}
          <div className="px-3 py-2.5 grid grid-cols-2 gap-2 border-b border-slate-50">
            <div className="relative">
              <Database size={11} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400"/>
              <select value={dbType} onChange={e=>setDbType(e.target.value)}
                className="w-full text-[11px] border border-slate-200 rounded-lg pl-7 pr-2 py-1.5 text-slate-600 bg-slate-50 outline-none focus:border-indigo-400 appearance-none">
                {['postgresql','mysql','mongodb','mssql','oracle','clickhouse'].map(t=>(<option key={t} value={t}>{t.toUpperCase()}</option>))}
              </select>
            </div>
            <select value={ctxConnId} onChange={e=>setCtxConnId(e.target.value)}
              className="w-full text-[11px] border border-slate-200 rounded-lg px-2 py-1.5 text-slate-600 bg-slate-50 outline-none focus:border-indigo-400">
              <option value="">No connection</option>
              {connections.map(c=>(<option key={c.id} value={c.id}>#{c.id} {c.connection_name||c.host}</option>))}
            </select>
          </div>

          {/* Prompt library (collapsible) */}
          <button onClick={()=>setToolOpen(toolOpen==='library'?null:'library')}
            className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-slate-50 transition-colors">
            <span className="flex items-center gap-2 text-xs font-bold text-slate-700"><Zap size={13} className="text-violet-500"/> Prompt library</span>
            <ChevronDown size={13} className={`text-slate-400 transition-transform ${toolOpen==='library'?'rotate-180':''}`}/>
          </button>
          {toolOpen==='library' && (
            <div className="max-h-56 overflow-y-auto bg-slate-50/60 border-t border-slate-100">
              <button onClick={()=>handleQuickSQL('create_table')}
                className="w-full flex items-center gap-2 px-4 py-2.5 text-[12px] font-bold text-violet-700 hover:bg-white transition-colors border-b border-slate-100">
                <Code2 size={12}/> Generate CREATE TABLE
              </button>
              {QUICK_LIBRARY.map(cat=>(
                <div key={cat.category}>
                  <button onClick={()=>setActiveLib(activeLib===cat.category?null:cat.category)}
                    ...
                    <span className="flex items-center gap-2">
                      <span className={`w-5 h-5 rounded-md border flex items-center justify-center ${colCls[cat.color]}`}>{cat.icon}</span>
                      <span className="text-[11px] font-bold text-slate-600">{cat.category}</span>
                    </span>
                    <ChevronRight size={11} className={`text-slate-400 transition-transform ${activeLib===cat.category?'rotate-90':''}`}/>
                  </button>
                  {activeLib===cat.category && cat.items.map(item=>(
                    <button key={item.label} onClick={()=>send(item.msg)}
                      className="w-full text-left pl-11 pr-4 py-2 text-[11.5px] text-slate-600 hover:bg-white hover:text-slate-900 transition-all">
                      {item.label}
                    </button>
                  ))}
                </div>
          ...
          <button onClick={()=>setToolOpen(toolOpen==='export'?null:'export')}
            ...
            <span className="flex items-center gap-2 text-xs font-bold text-slate-700"><FileText size={13} className="text-indigo-500"/> Export &amp; reports</span>
            <ChevronDown size={13} className={`text-slate-400 transition-transform ${toolOpen==='export'?'rotate-180':''}`}/>
          </button>
          {toolOpen==='export' && (
            <div className="px-3 py-2.5 space-y-1.5 bg-slate-50/60 border-t border-slate-100">
              <button onClick={()=>generatePDFReport(messages, connections)} disabled={messages.length===0}
                className="w-full flex items-center gap-2 px-3 py-2 rounded-xl bg-indigo-50 border border-indigo-200 text-[11px] font-bold text-indigo-700 hover:bg-indigo-100 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                <FileText size={11}/> Export current chat as PDF
              </button>
              {[
                ...
                <button key={type} onClick={()=>downloadReport(type,conn||undefined)}
                  className="w-full flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-[11px] font-semibold text-emerald-700 hover:bg-emerald-100 transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#f8fafc]">
        ...
        <div className="flex-shrink-0 px-6 pb-6 pt-3 bg-white border-t border-slate-100">
          <div className="max-w-3xl mx-auto flex items-end gap-3 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3
