...
/* draggable-launcher constants/helpers */
const LAUNCH = 60;                                   // launcher button size (px)
const POS_KEY = 'actmon_ai_btn_pos';
const clampN = (v, min, max) => Math.max(min, Math.min(max, v));
const defaultPos = () => (typeof window !== 'undefined')
  ? { x: window.innerWidth - LAUNCH - 24, y: window.innerHeight - LAUNCH - 24 }
  : { x: 24, y: 24 };
      ...
      <div className="flex-1 min-w-0">
        <div className="bg-white border border-emerald-100 rounded-2xl rounded-tl-md shadow-sm px-4 py-3">
          <p className="text-[13px] text-slate-700 mb-2">
            Navigating to <strong>{nav.title || 'page'}</strong>
          </p>
          <button
            onClick={() => { navigate(nav.path); onClose(); }}
...
export default function ChatbotWidget() {
  ...
  const [pos, setPos] = useState(() => {
    try { const s = JSON.parse(localStorage.getItem(POS_KEY) || 'null'); if (s && typeof s.x === 'number') return s; } catch (_) {}
    return defaultPos();
  });
  const drag = useRef({ dragging: false, moved: false, sx: 0, sy: 0, px: 0, py: 0 });

  const onLaunchDown = (e) => {
    e.currentTarget.setPointerCapture?.(e.pointerId);
    drag.current = { dragging: true, moved: false, sx: e.clientX, sy: e.clientY, px: pos.x, py: pos.y };
  };
  const onLaunchMove = (e) => {
    const d = drag.current;
    if (!d.dragging) return;
    const dx = e.clientX - d.sx, dy = e.clientY - d.sy;
    if (Math.abs(dx) > 4 || Math.abs(dy) > 4) d.moved = true;
    setPos({
      x: clampN(d.px + dx, 6, window.innerWidth - LAUNCH - 6),
      y: clampN(d.py + dy, 6, window.innerHeight - LAUNCH - 6),
    });
  };
  const onLaunchUp = (e) => {
    const d = drag.current;
    ...
    if (d.moved) { try { localStorage.setItem(POS_KEY, JSON.stringify(pos)); } catch (_) {} }
    else setOpen(true);   // treat as a click when not dragged
  };

  // keep the launcher on-screen when the window is resized
  useEffect(() => {
    const onResize = () => setPos((p) => ({
      x: clampN(p.x, 6, window.innerWidth - LAUNCH - 6),
      y: clampN(p.y, 6, window.innerHeight - LAUNCH - 6),
    }));
  ...
  const send = useCallback(async (text = input.trim()) => {
    ...
    // ── Navigation command detection (any page, natural language) ──
    ...
    if (navResult) {
      ...
      setTimeout(() => { navigate(navResult.path); setOpen(false); }, 700);
      return;
    }
  ...
  // Let any page open the assistant — optionally with a pre-filled question — via
  //   window.dispatchEvent(new CustomEvent('actmon:open-chat', { detail: { ask } }))
  // Used by the Help Desk / Documentation ("Ask ActMon AI") buttons.
  useEffect(() => {
    const onOpen = (e) => {
      setOpen(true);
      const ask = e?.detail?.ask;
      if (ask) setTimeout(() => send(String(ask)), 350);
    };
    window.addEventListener('actmon:open-chat', onOpen);
    return () => window.removeEventListener('actmon:open-chat', onOpen);
  }, [send]);
  ...
  // ── Closed: draggable floating icon ─────────────────
  if (!open) {
    ...
    return (
      <button
        ...
        title="ActMon AI · click to open, drag to move"
        className="fixed z-[60] group flex items-center justify-center rounded-full text-white shadow-2xl hover:scale-105 active:scale-95 transition-transform touch-none select-none"
        style={{ left: pos.x, top: pos.y, width: LAUNCH, height: LAUNCH, cursor: drag.current.dragging ? 'grabbing' : 'grab',
          background:'linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)', boxShadow:'0 10px 34px rgba(79,70,229,0.45)' }}
      >
        {/* pulsing ring */}
        <span className="absolute inset-0 rounded-full animate-ping" style={{ background:'rgba(124,58,237,0.35)', animationDuration:'2.4s' }} />
        <Brain size={26} className="relative"/>
        {msgCount > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1 rounded-full bg-white text-indigo-700 text-[11px] font-black flex items-center justify-center shadow ring-2 ring-indigo-100">
            {msgCount}
          </span>
        )}
      </button>
  ...
  const H = Math.min(620, (typeof window !== 'undefined' ? window.innerHeight : 720) - 32);
  // anchor the panel to the launcher position, clamped inside the viewport
  const vw = typeof window !== 'undefined' ? window.innerWidth : 1280;
  const vh = typeof window !== 'undefined' ? window.innerHeight : 720;
  let panelLeft = pos.x + LAUNCH - W;                 // align panel's right edge with the icon
  let panelTop  = pos.y - H - 12;                     // prefer opening above the icon
  if (panelTop < 16) panelTop = pos.y + LAUNCH + 12;  // otherwise below
  panelLeft = clampN(panelLeft, 16, vw - W - 16);
  panelTop  = clampN(panelTop, 16, vh - H - 16);

  // ── Open: chat panel ───────────────────────────────
  return (
    <div
      className="fixed z-[60] flex flex-col rounded-2xl shadow-2xl overflow-hidden transition-all duration-300"
      style={{ left: panelLeft, top: panelTop, width:W, height:H, background:'var(--color-surface)', border:'1px solid rgba(99,102,241,0.15)' }}
    >
      {/* ── Header ───────────────────────────────────── */}
