...
export default function OracleLiveQueriesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] flex flex-col">
      ...
      {/* ─── HEADER ─── */}
      <div className="text-white shadow-xl sticky top-0 z-40"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-[18px] font-black tracking-tight">Live Query Monitor</h1>
                ...
                <p className="text-[12px] mt-0.5" style={{ color: 'rgba(252,165,165,0.8)' }}>
                  {conn.name || 'Oracle'} &mdash; {conn.host}:{conn.port}
                  {conn.service_name ? ` / ${conn.service_name}` : ''}
                </p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="pl-8 pr-3 py-1.5 rounded-lg text-xs bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:bg-white/20"
                style={{ width: 180 }} />
            </div>
            <button onClick={() => refetch()}
              ...
              <span className="w-5 h-5 rounded-full text-[10px] font-black flex items-center justify-center"
                style={{ background: 'rgba(199,70,52,0.4)', color: '#fca5a5' }}>{countdown}</span>
            </button>
          </div>
        </div>
      </div>
