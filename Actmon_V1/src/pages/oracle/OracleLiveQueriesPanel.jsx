...
import {
  ...
  Copy, CheckCheck, Search, Database,
  TrendingUp, Lock, FileText, BarChart2, Info,
} from 'lucide-react';
import {
...
const WAIT_COLORS = {
  'User I/O': C.orange, 'System I/O': C.blue, 'Concurrency': C.red,
  'Network': C.teal, 'CPU': C.green, 'Application': C.purple, 'Other': C.slate,
};
...
/* Embeddable Live Queries panel — used as a tab inside OracleDashboard, same as
   every other tab, instead of navigating away to a separate page. Content is
   unchanged from the original standalone page; only the outer PageHeader/route
   chrome was removed so it fits the dashboard shell like Slow Queries and the
   rest do. */
export default function OracleLiveQueriesPanel({ connId }) {
  const id = connId;
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="pl-8 pr-3 py-1.5 w-48 rounded-lg text-xs bg-white border border-slate-200 focus:outline-none focus:ring-2 focus:ring-red-200" />
          </div>
          <button onClick={() => refetch()}
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold bg-white border border-slate-200 hover:bg-slate-50">
            <RefreshCw size={13} className={isFetching ? 'animate-spin' : ''} />
            <span className="w-5 h-5 rounded-full text-[10px] font-black flex items-center justify-center bg-slate-100 text-slate-600">{countdown}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-6 gap-3">
        {[
          ...
          { label: 'Active Now', value: active, color: active > 0 ? C.green : C.slate, icon: Activity, sub: active > 0 ? 'executing SQL' : 'none executing' },
          { label: 'Idle Sessions', value: inactive, color: C.slate, icon: Clock, sub: inactive > 0 ? 'connected, idle' : '—' },
          { label: 'Blocking', value: blocking, color: blocking > 0 ? C.red : C.green, icon: Lock, sub: blocking > 0 ? 'lock chains!' : 'no locks' },
          { label: 'Max Wait (s)', value: maxWait > 0 ? maxWait.toFixed(1) : '—', color: maxWait > 30 ? C.red : maxWait > 5 ? C.orange : C.slate, icon: TrendingUp, sub: maxWait > 30 ? 'critical!' : maxWait > 5 ? 'elevated' : 'normal' },
          { label: 'Auto-Refresh', value: `${countdown}s`, color: C.teal, icon: RefreshCw, sub: 'every 5s' },
        ].map(({ label, value, color, icon: Icon, sub }) => (
          <div key={label} className="bg-white rounded-xl border border-slate-200 p-4" style={{ borderLeft: `3px solid ${color}` }}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 xl:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={120}>
            <AreaChart data={history}>
              <defs>
                {[['grad-a', C.green], ['grad-w', C.orange], ['grad-m', C.red]].map(([gid, c]) => (
              ...
              <Tooltip contentStyle={{ fontSize: 10, padding: '4px 8px', borderRadius: 8 }} labelFormatter={v => `Time: ${v}`} />
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          {waitPieData.length > 0 ? (
            <ResponsiveContainer width="100%" height={140}>
              <PieChart>
                <Pie data={waitPieData} dataKey="value" nameKey="name" innerRadius={35} outerRadius={55}
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {waitPieData.map((d) => (
              <span key={d.name} className="text-[10px] flex items-center gap-1">
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: WAIT_COLORS[d.name] || '#64748B' }} />
                {d.name} ({d.value})
              </span>
            ))}
          </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3" style={{ borderLeft: '3px solid #C74634' }}>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <Activity size={14} style={{ color: C.red }} />
              <h3 className="font-black text-slate-800 text-sm uppercase tracking-tight">Sessions ({filtered.length})</h3>
            ...
            <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
              {[['ALL', `All (${queries.length})`], ['ACTIVE', `Active (${active})`], ['INACTIVE', `Idle (${inactive})`]].map(([v, label]) => (
                <button key={v} onClick={() => setStatusFilter(v)}
                  className={`px-2.5 py-1 rounded-md text-[10px] font-bold transition-all ${statusFilter === v ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{label}</button>
              ))}
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-xs text-slate-400 mt-1">
              {queries.length === 0 ? 'No user sessions found — Oracle may be idle or no clients are connected' : `${queries.length} total sessions — try a different filter`}
            </p>
          </div>
        ) : (
          <div>
            <div className="grid gap-0 text-[10px] font-bold text-slate-400 uppercase tracking-wide bg-slate-50 border-b border-slate-200 px-4 py-2"
              ...
              <span>SID</span><span>Username</span><span>Status</span><span>SQL ID</span><span>Wait / State</span>
              <span>Wait/Idle</span><span>Machine</span><span>SQL Preview</span><span></span>
            </div>
            ...
            {filtered.map((q, i) => {
              ...
              return (
                <div key={i} className={`border-b border-slate-100 ${isExp ? 'bg-slate-50' : ''} ${!isActive ? 'opacity-75' : ''}`}>
                  <div
                    className={`grid items-center gap-0 px-4 py-2.5 cursor-pointer hover:opacity-100 hover:bg-slate-50/80 transition-all text-xs ${q.blocking_session ? 'bg-red-50/50' : ''}`}
                    style={{ gridTemplateColumns: '55px 100px 85px 105px 170px 75px 120px 1fr 40px' }}
                    onClick={() => toggleExpand(i, q.sql_id || q.prev_sql_id)}>

                    <span className="font-mono text-slate-500 text-[11px]">{q.sid}</span>
                    <span className={`font-black truncate text-[11px] ${isActive ? 'text-red-700' : 'text-slate-500'}`}>{q.username || 'SYS'}</span>
                    <span>
                      {isActive ? (
                        ...
                        <span className="text-[10px] font-bold text-slate-400">idle {fmtIdle(idleSecs)}</span>
                      )}
                    </span>
                    ...
                    <span className="flex items-center gap-1.5">
                      {isActive ? (
                        <>
                          ...
                          <span className="truncate text-[10px]" style={{ color: wcolor }}>{q.wait_event || 'On CPU'}</span>
                        </>
                      ) : (
                        <span className="text-[10px] text-slate-400 italic">{q.state || 'WAITING'} for client</span>
                      )}
                    </span>
                    <span className={`font-black text-sm ${isActive && waitSecs > 30 ? 'text-red-600' : isActive && waitSecs > 5 ? 'text-orange-500' : isActive ? 'text-green-600' : 'text-slate-400'}`}>
                      {isActive && waitSecs > 0 ? `${waitSecs}s` : !isActive && idleSecs > 0 ? fmtIdle(idleSecs) : '—'}
                    </span>
                    <span className="text-[10px] text-slate-400 truncate">{(q.machine || '').split('\\').pop()?.slice(0, 16) || '—'}</span>
                    <span className="font-mono text-[10px] text-slate-500 truncate">
                      {isActive ? (effectiveSql || '—').slice(0, 65) : effectiveSql ? `[last] ${effectiveSql.slice(0, 55)}` : '(no recent SQL)'}
                    </span>
                        ...
                        <div className="flex items-center justify-between px-4 py-2 border-b border-slate-700">
                          <div className="flex items-center gap-2">
                            ...
                            {!isActive && q.prev_sql_id && (
                              <span className="text-[9px] px-1.5 py-0.5 rounded bg-slate-700 text-slate-400">prev_sql_id: {q.prev_sql_id}</span>
                            )}
                          </div>
                          <button
