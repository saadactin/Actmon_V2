...
export default function MSSQLSlowQueries() {
  ...
  return (
    <div className="min-h-full bg-brand-bg">
      ...
      <PageHeader
        icon={Zap}
        title="MSSQL Slow Queries"
        subtitle="Source: sys.dm_exec_query_stats — top queries by elapsed time"
        accent="mssql"
        backTo={`/mssql-dashboard/${id}`}
        crumbs={[{ label: 'Databases', to: '/databases' }, { label: 'SQL Server', to: `/mssql-dashboard/${id}` }, { label: 'Slow Queries' }]}
        actions={(
